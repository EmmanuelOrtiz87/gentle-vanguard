...
export function getMCPServersStatus(): MCPServerEntry[] {
  if (existsSync(GATEWAY_SCRIPT)) {
    ...
    if (raw) {
      try { return JSON.parse(raw.trim())?.servers || []; } catch { /* fallthrough */ }
    }
  }
  ...
  return (registry.servers || []).map((s: any) => ({
    name: s.name, type: s.type || 'user', transport: s.transport || 'stdio',
    command: s.command, args: s.args || [], enabled: s.enabled !== false,
    autoStart: s.autoStart || false, description: s.description || '',
    pid: null, status: 'unknown', uptime: 0, toolsCount: 0, lastError: null,
  }));
}

export function mcpServersHandler(_req: IncomingMessage, res: ServerResponse, headers: Record<string, string>) {
  const servers = getMCPServersStatus();
  res.writeHead(200, headers);
  res.end(JSON.stringify({ type: 'mcp-servers', data: { servers } }));
}

export function mcpServerActionHandler(req: IncomingMessage, res: ServerResponse, headers: Record<string, string>) {
  const url = new URL(req.url!, `http://${req.headers.host || 'localhost'}`);
  const match = url.pathname.match(/^\/api\/mcp\/servers\/([^/]+)\/(start|stop)$/);
  if (!match) { res.writeHead(404); res.end('Not found'); return; }
  const [, name, action] = match;
  if (!existsSync(MANAGER_SCRIPT)) { res.writeHead(500); res.end('Manager not found'); return; }
  pwsh(`& '${MANAGER_SCRIPT}' -Action ${action} -Name '${name}' -Quiet`);
  res.writeHead(200, headers);
  res.end(JSON.stringify({ success: true, name, action }));
}

export function mcpServerRegisterHandler(req: IncomingMessage, res: ServerResponse, headers: Record<string, string>) {
  let body = '';
  ...
  req.on('end', () => {
    try {
      ...
      if (!payload.name || !payload.command) {
        res.writeHead(400, headers); res.end(JSON.stringify({ error: 'name and command required' }));
        return;
      }
