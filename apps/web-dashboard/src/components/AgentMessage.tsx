...
import { Bot, User, CheckCircle, XCircle, Clock, ChevronDown, ChevronRight, Loader2, AlertTriangle, Info, FileDiff, FormInput } from 'lucide-react';
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      {expanded && (
        <div className="px-3 py-2 text-xs font-mono bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
          {call.args && <div className="mb-1"><span className="text-gray-500">args:</span> {JSON.stringify(call.args, null, 2)}</div>}
          {call.result && <div className="mb-1"><span className="text-green-600">result:</span> {call.result.slice(0, 200)}</div>}
          {call.error && <div><span className="text-red-600">error:</span> {call.error}</div>}
        </div>
      )}
    </div>
  );
          ...
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-800/50">
              {hint.columns.map((col, i) => (
                <th key={i} className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">{col}</th>
              ))}
            </tr>
          </thead>
          <tbody>
    ...
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className="grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-700">
        <div className="p-3">
          ...
          <pre className="text-xs text-red-600 dark:text-red-400 whitespace-pre-wrap font-mono bg-red-50 dark:bg-red-900/20 p-2 rounded">{hint.oldValue}</pre>
        </div>
        <div className="p-3">
          <p className="text-[10px] font-medium text-gray-500 mb-1">After</p>
          <pre className="text-xs text-green-600 dark:text-green-400 whitespace-pre-wrap font-mono bg-green-50 dark:bg-green-900/20 p-2 rounded">{hint.newValue}</pre>
        </div>
      </div>
    </div>
  );
...
function FormHint({ hint, onAction }: { hint: UIHint; onAction?: (action: string, values: Record<string, unknown>) => void }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{hint.label || 'Form'}</span>
      </div>
...
function MetricHint({ hint }: { hint: UIHint }) {
  ...
  const colorClass = hint.severity ? colorMap[hint.severity] : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300';
...
function AlertHint({ hint }: { hint: UIHint }) {
  ...
  const sevColor = hint.severity === 'error' ? 'border-red-500 bg-red-50 dark:bg-red-900/20' :
...
function UIHintBadge({ hint, onFormAction }: { hint: UIHint; onFormAction?: (action: string, values: Record<string, unknown>) => void }) {
  switch (hint.type) {
    case 'metric':
...
export function AgentMessage({ message, onFormAction }: AgentMessageProps) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''} ${isSystem ? 'opacity-70' : ''}`}>
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
        isUser ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' :
        isSystem ? 'bg-gray-100 dark:bg-gray-800 text-gray-500' :
      ...
      <div className={`flex-1 max-w-[80%] ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
        <div className={`rounded-xl px-4 py-2.5 ${
          ...
          {!message.streaming && <span className="text-sm whitespace-pre-wrap">{message.content}</span>}
        </div>
