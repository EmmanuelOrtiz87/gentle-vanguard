...
function Get-Stats {
    if (Test-Path $StatsFile) {
        ...
        catch { return @{ hits = @{L1=0;L2=0;L3=0;Archive=0}; misses = 0; sets = 0; evictions = 0; startedAt = (Get-Date -Format 'o') } }
    }
...
function New-CacheEntry {
    ...
    return @{
        ...
        compressed = if ($Compress) { $true } else { $false }
    }
}
...
function Get-AutoTier {
    ...
    if ($ttl -le 300)      { return 'L1' }
    if ($ttl -le 3600)     { return 'L2' }
    if ($size -gt 1048576) { return 'Archive' }
    return 'L3'
}
...
function Invoke-Get {
    ...
    if ($entry) {
        ...
        Set-L1 $k $entry.value $entry.ttl ($entry.tags -join ',')
    ...
    if ($entry) {
        ...
        Set-L1 $k $entry.value $entry.ttl ($entry.tags -join ',')
        Set-L2 $k $entry.value $entry.ttl ($entry.tags -join ',')
        Write-Output $entry.value; return
    }
    $entry = Get-Archive $k
    if ($entry) {
    ...
    if ($Tag) {
        ...
        foreach ($dir in @($L2Dir, $L3Dir, $ArchiveDir)) {
            Get-ChildItem $dir -Filter '*.cache.json' -ErrorAction SilentlyContinue | ForEach-Object {
                $p = $_.FullName
                try {
                    $entry = Get-Content $p -Raw | ConvertFrom-Json
                    $entryTags = @($entry.tags)
                    if ($tags | Where-Object { $entryTags -contains $_ }) { Remove-Item $p -Force; $removed++ }
                } catch { Remove-Item $p -Force; $removed++ }
            }
        }
        $toRemove = @($L1.Keys | Where-Object {
            $e = $L1[$_]
            $et = @($e.tags)
            ($tags | Where-Object { $et -contains $_ }) -ne $null
        })
...
function Invoke-GC {
    ...
    foreach ($dir in @($L2Dir, $L3Dir)) {
        Get-ChildItem $dir -Filter '*.cache.json' -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            try {
                $entry = Get-Content $p -Raw | ConvertFrom-Json
                if (Test-Expired $entry) { Remove-Item $p -Force; $removed++; Increment-Stat 'evict' }
            } catch { Remove-Item $p -Force; $removed++ }
        }
    }
