...
function Collect-PRMetrics {
    ...
    } catch { $e = $_.Exception.Message; Log "PR: collection failed: $e" }
    $prMetrics | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $outDir 'pr.json')
    Log "PR: $($prMetrics.total) total, $($prMetrics.merged) merged, avg $($prMetrics.avgReviewTimeHours)h lifecycle"
    return $prMetrics
}
...
function Collect-SessionMetrics {
    ...
    if (Test-Path $sessionsDir) {
        Get-ChildItem -Path $sessionsDir -Filter 'session-*.json' | ForEach-Object {
            ...
            } catch { $e = $_.Exception.Message; Write-Debug "Session parse skipped: $e" }
        }
    }
    ...
    Log "Sessions: $total total, $active active, $todaySessions today, ${totalDurMin}m total"
    return $sessions
}
...
function Collect-TokenMetrics {
    ...
    if (Test-Path $tokenState) { try { $t = Get-Content $tokenState -Raw | ConvertFrom-Json; $tm.status = $t.lastStatus } catch { $e = $_.Exception.Message; Write-Debug "Token state not read: $e" } }
