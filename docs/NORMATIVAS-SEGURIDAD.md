...
> - Se expandieron patrones críticos en `security-orchestrator.ps1` (jailbreak, leakage, code execution)
...
1. **MUST** detect and BLOCK instruction override attempts en todo input de usuario antes de pasarlo al agente
...
7. **MUST** detectar constraint bypass (forget instructions, disregard safeguards, override protocols)
