...
function main(): void {
  ...
  const log = (m: string, l?: string) => { if (!quiet) console.log(l ? `[${l}] ${m}` : `[INFO] ${m}`); };
  ...
  if (existsSync(metaPath)) {
    try { prevMeta = JSON.parse(readFileSync(metaPath, 'utf-8')); } catch { log('Could not parse existing metadata, doing full rebuild', 'WARN'); }
  }

  // Load existing embeddings
  let prevEmbeddings: Record<string, unknown> | null = null;
  if (existsSync(outputPath)) {
    try { prevEmbeddings = JSON.parse(readFileSync(outputPath, 'utf-8')); } catch { log('Could not parse existing embeddings, doing full rebuild', 'WARN'); }
  }

  if (!prevMeta || !prevEmbeddings) log('No previous embeddings found, doing full rebuild');

  if (!existsSync(registryPath)) { log('Skill registry not found', 'ERROR'); process.exit(1); }
  ...
  for (const line of registryContent.split(/\r?\n/)) {
    ...
    if (m) { const n = m[1]; const a = m[2]; if (n !== '---' && n !== 'Skill') currentSkills[n] = a; }
  }
  ...
  if (existsSync(delegationConfigPath)) {
    try {
      ...
      if (config.agents) {
        for (const [agentName, agentData] of Object.entries(config.agents) as [string, { skills?: Record<string, unknown> }][]) {
          if (agentData.skills) {
  ...
  if (prevMeta && (prevMeta as Record<string, unknown>).contentHashes) {
    for (const [k, v] of Object.entries((prevMeta as Record<string, unknown>).contentHashes as Record<string, string>)) {
      prevSkills[k] = v;
    }
  }
  ...
  const changePercent = Object.keys(currentSkills).length > 0 ? (totalChanged / Object.keys(currentSkills).length) * 100 : 100;

  log(`DIFF: Added: ${added.length} | Removed: ${removed.length} | Modified: ${modified.length} | Unchanged: ${unchanged.length}`);
  ...
  if (dryRun) { log(`[DRY-RUN] Would add: ${added.join(', ')}`); log(`[DRY-RUN] Would remove: ${removed.join(', ')}`); log(`[DRY-RUN] Would modify: ${modified.join(', ')}`); return; }
  ...
  if (modified.length > 0 || added.length > 0) {
    ...
    if (prevEmbeddings && (prevEmbeddings as Record<string, unknown>).skills) {
      ...
      for (const skill of (prevEmbeddings as Record<string, unknown>).skills as Array<Record<string, unknown>>) {
      ...
      const embeddings = {
        ...
        metadata: { totalSkills: existingSkills.size, vocabularySize: ((prevEmbeddings as Record<string, unknown>).metadata as Record<string, number>)?.vocabularySize || 0, ngramSize: 3 },
  ...
  // Log
  try { appendFileSync(logFile, JSON.stringify({ timestamp: new Date().toISOString(), action: 'incremental', added, removed, modified, totalSkills: Object.keys(currentSkills).length }) + '\n', 'utf-8'); } catch { /* */ }

  // Update metadata
  const hashes: Record<string, string> = {};
  for (const [skill, agent] of Object.entries(currentSkills)) hashes[skill] = sha16(`${skill}|${agent}`);

  const meta: Record<string, unknown> = {
    version: '1.0',
    lastBuilt: new Date().toISOString(),
    lastFullRebuild: (prevMeta as Record<string, string>)?.lastFullBuild || new Date().toISOString(),
    totalSkills: Object.keys(currentSkills).length,
    vocabularySize: prevEmbeddings ? ((prevEmbeddings as Record<string, unknown>).metadata as Record<string, number>)?.vocabularySize || 0 : 0,
    contentHashes: hashes,
    incrementalUpdates: [...((prevMeta as Record<string, unknown>)?.incrementalUpdates as Array<unknown>) || [], { timestamp: new Date().toISOString(), added, removed, modified }],
  };
  writeFileSync(metaPath, JSON.stringify(meta, null, 2), 'utf-8');

  log(`Incremental update complete. Skills: ${Object.keys(currentSkills).length} | Added: ${added.length} | Removed: ${removed.length} | Modified: ${modified.length}`, 'OK');
}
