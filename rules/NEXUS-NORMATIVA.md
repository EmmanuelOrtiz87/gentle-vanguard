...
**Name:** Nexus **Tagline:** El punto central donde converge toda la información operacional del
stack. **File:** `.runtime/gentle-vanguard.db` **Type:** SQLite (WAL mode, foreign keys ON)
...
El componente `gentle-vanguard-db` en la watchtower (`src/core/maintenance-watchtower.ts`) verifica
en cada ciclo de auto-heal:
