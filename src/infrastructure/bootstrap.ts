...
/**
 * Registers a hidden Windows scheduled task that runs a TS script via
 * `node --import tsx <script>` every `intervalHours` hours.
 *
 * Hidden execution: node.exe directly (no pwsh, no npx.cmd chain) with an S4U
 * principal so the Task Scheduler never shows a console window, even while the
 * user is logged on interactively. Registration is idempotent (-Force) and
 * migrates the old visible pwsh.exe form on existing installs.
 *
 * Returns 'ok' (S4U registered), 'fallback' (wscript wrapper), or 'error'.
 */
function registerScheduledTask(
  ...
  const vbsPath = join(root, '.runtime', `${taskName.toLowerCase().replace(/[^a-z0-9]/g, '-')}.vbs`);
  const vbsContent =
