---
name: huashu-design
description: High-fidelity HTML design skill: prototypes, slides/decks (with editable PPTX export via html2pptx), animations, infographics and 5-dimension expert review. Always drafts 3 design directions first. Triggers: design prototype, deck design, make a PPT, high-fidelity mockup, export PPTX, infographic, design review, 'diseño de deck', 'prototipo', 'infografía'. 花叔Design——用HTML做高保真原型、幻灯片、动画、可视化与专家评审。任何新设计100%先出三个方向初稿给用户选（指定风格/品牌也不豁免），选定后才执行。触发词：做原型、PPT、幻灯片、动画、设计风格、评审、做个HTML页面、UI mockup、导出MP4/GIF、做个好看的。生产级Web App/需后端的系统不适用。
metadata:
  source: external-adopted
  upstream: alchaincyf/huashu-design
  license: MIT
  note: 'bgm-*.mp3 excluded (25MB non-essential)'
---
