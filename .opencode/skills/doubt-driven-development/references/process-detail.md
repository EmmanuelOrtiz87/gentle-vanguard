...
In Claude Code, the role-based reviewers in `agents/` start with isolated context by design. **The
adversarial prompt above takes precedence over the persona's default response shape.** Personas like
`code-reviewer` are written to produce balanced verdicts; doubt-driven needs issues-only output.
Paste the adversarial prompt verbatim into the invocation so it overrides the persona's default.

### Cross-model escalation

For full detail including CLI invocation safety, tool verification, and non-interactive handling,
see [cross-model-escalation.md](cross-model-escalation.md).
...
1. **Contract misread** — reviewer flagged something because the CONTRACT was unclear or incomplete.
   Fix the contract first, re-classify on the next cycle.
...
4. **Noise** — reviewer flagged something correct under context the reviewer didn't have. Note it,
   move on, and ask: would adding that context to the contract have prevented the false flag?
