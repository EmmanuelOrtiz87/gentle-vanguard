...
try {
    ...
    const result = spawnSync('npx', ['tsx', 'src/token-budget-guard.ts', '-Mode', 'check', '-Task', 'factorial-test', '-Risk', 'low', '-EstimatedChars', '100', '-Record', '-Quiet'], {
        cwd: ROOT,
        stdio: 'pipe',
        encoding: 'utf-8'
    });
...
console.log(`   Ahorro:       ${dailyUsage.reduction}% (${dailyUsage.savings.toLocaleString()} tokens/día)`);
...
console.log(`Tokens: ${realExample.originalTokens} → ${realExample.optimizedTokens} (-${realExample.reduction}%)`);
