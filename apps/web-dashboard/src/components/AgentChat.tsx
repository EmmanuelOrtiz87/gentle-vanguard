...
import { Send, Plus, MessageSquare, Bot, Wifi, WifiOff, Loader2, Terminal, History, Sparkles, Bug, TestTube } from 'lucide-react';
...
const SUGGESTED_ACTIONS = [
  { label: 'Run tests', icon: TestTube, query: 'Execute the test suite and report results', agent: 'QA' },
  { label: 'Check skills', icon: Terminal, query: 'List all available skills and their status', agent: 'DEV' },
  { label: 'Review logs', icon: Bug, query: 'Review recent agent logs for errors or warnings', agent: 'OPS' },
  { label: 'Analyze', icon: Sparkles, query: 'Analyze the current project state and suggest improvements', agent: 'BA' },
];
...
export default function AgentChat() {
  ...
  const insertMention = useCallback((code: string) => {
  ...
  const filteredAgents = mentionQuery
    ? AGENTS.filter(a => a.code.toLowerCase().includes(mentionQuery) || a.name.toLowerCase().includes(mentionQuery))
    : AGENTS;
  ...
  const handleSuggestedAction = useCallback((query: string, agent: string) => {
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-4">
      <div className="flex-1 flex flex-col bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          <div className="flex items-center gap-2">
            ...
            {bridgeConnected && <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-0.5 rounded-full">MCP</span>}
            <button onClick={handleNewSession} className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" title="New session">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50 dark:bg-gray-900/50">
          {!session && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400 mb-2">No active session</h3>
              <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">Select an agent and start a conversation</p>
            ...
            {mentionIndex !== -1 && filteredAgents.length > 0 && (
              <div className="absolute bottom-full left-0 mb-1 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden z-10">
                {filteredAgents.map((a) => (
                  ...
                  >
                    <span className="font-mono font-medium text-purple-600 dark:text-purple-400">@{a.code}</span>
      ...
      <div className="w-72 flex flex-col gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-3">
          <button
            onClick={() => { listSessions(); setShowSessions(!showSessions); }}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-3">
          <button
            onClick={() => { listTools(); setShowTools(!showTools); }}
            className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
          >
          ...
          {showTools && (
            <div className="space-y-1 max-h-48 overflow-y-auto">
              ...
              {bridgeConnected && tools.length === 0 && <p className="text-xs text-gray-400">No tools</p>}
              {tools.map((t) => (
                <button
                  key={t.name}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-3">
          <button
            onClick={() => { listHistory(); setShowHistory(!showHistory); }}
            className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
          >
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              ...
              {historySessions.map((s: AgentSession) => (
                ...
                >
                  ...
                  <span className="text-gray-400 block text-[10px]">{new Date(s.updatedAt).toLocaleString()}</span>
                </button>
              ))}
            </div>
          )}
      ...
      <HitlModal
        ...
        onDismiss={() => {/* Modal stays until resolved */}}
      />
    </div>
  );
}
