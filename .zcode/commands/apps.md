...
- Estado: `curl -s http://127.0.0.1:8090/api/apps` — lista id, status (running/stopped/partial) y
  PID+puerto por proceso.
...
Al reportar al usuario: tabla app → status → URL, y señala cualquier `partial` (mirar sus logs con
el endpoint de logs antes de reiniciar).
