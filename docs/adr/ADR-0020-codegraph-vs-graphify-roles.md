...
- **`.codegraph/`** (`codegraph.db`): índice incremental mantenido por hooks
  (post-commit/post-merge) y tarea programada horaria. Consumido por el MCP server
  `codegraph` (tooling MCP: `codegraph query/explain/path/affected`).
...
consumidores distintos. Sin roles explícitos, el mantenimiento se duplica y las interfaces
divergen sin criterio.
...
1. **No duplicar mantenimiento**: los hooks y la tarea programada mantienen CodeGraph; el builder
   de graphify se ejecuta tras cambios estructurales (moves de archivos, refactors grandes) y en el
   pipeline de sesión cuando el grafo está ausente o stale.
2. **No fusionar**: graphify NO consume el índice de CodeGraph (formatos y propósitos distintos);
   la fusión se reevaluará solo si un consumidor demuestra necesidad de ambos en una sola query.
