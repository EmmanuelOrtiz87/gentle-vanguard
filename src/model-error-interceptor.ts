...
const ERROR_PATTERNS = {
  quota: [
    ...
    'credits exhausted', 'free usage exceeded', 'subscribe',
    'payment required', 'billing limit'
  ],
  modelNotFound: [
    'model not found', 'invalid model', 'unknown model',
    'not available', 'inherit-from-session'
  ],
  timeout: [
    'timeout', 'deadline exceeded', 'request timed out',
    'connection timeout', 'unreachable', 'econnrefused'
  ],
  auth: [
...
export async function interceptTaskError(
  ...
  if (fallbackId) {
    ...
    if (fallbackEntry) {
      // Update active model
      writeFileSync(ACTIVE_MODEL_PATH, JSON.stringify({
        model: fallbackId,
