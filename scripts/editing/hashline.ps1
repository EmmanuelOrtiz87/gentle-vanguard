...
function Read-Db {
    if (Test-Path $HashDb) {
        try { return Get-Content $HashDb -Raw -Encoding UTF8 | ConvertFrom-Json -AsHashtable } catch { Write-Debug "Exception caught: param(
    [ValidateSet('init', 'verify', 'update', 'status', 'prune')]
    [string]$Action = 'status',
    [string]$Path = '',
    [string]$LineId = '',
...
function Read-Db {
    if (Test-Path $HashDb) {
        try { return Get-Content $HashDb -Raw -Encoding UTF8 | ConvertFrom-Json -AsHashtable } catch { Write-Output "[editing] Operation failed, continuing" }
    }
