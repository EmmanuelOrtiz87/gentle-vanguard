...
function recordFindingsLedger(event: EscalationEvent): void {
  ...
  writeFileSync(findingFile, JSON.stringify({
    id: event.id,
    type: 'escalation',
...
export function escalate(component: string, description: string, currentState: string): EscalationEvent {
...
export function getEscalationStatus(): { components: Record<string, number>; activeEscalations: number } {
  const history = loadFailureHistory();
...
function main(): void {
  ...
  if (action === 'status') {
    ...
    console.log('  Commands: status, escalate <component> <description> [state], clear <component>');
  }
}
