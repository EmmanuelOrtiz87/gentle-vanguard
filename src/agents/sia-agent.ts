...
const AGENT_CONFIG: DomainAgentConfig = {
  ...
  execute: (task, context, prompt) => {
    ...
    const intent =
      normalized.includes('review') || normalized.includes('feedback') ? 'REVIEW' :
      normalized.includes('score') || normalized.includes('score') ? 'SCORE' :
      normalized.includes('iterate') || normalized.includes('revise') ? 'ITERATE' :
      normalized.includes('escalat') ? 'ESCALATE' :
      'SIA_LOOP';
    ...
    if (score !== null && score >= 80) {
      flags.push({ severity: 'info', message: `Score ${score} >= 80 — iteration complete, no further revision needed.` });
    } else if (score !== null) {
      flags.push({ severity: 'warn', message: `Score ${score} < 80 — continue to feedback. Feedback must be specific, actionable, prioritized (top 3).` });
    }
    if (iteration && parseInt(iteration, 10) >= 5) {
      flags.push({ severity: 'critical', message: 'Iteration 5 reached — escalate to orchestrator if score < 60 or no improvement.' });
    }

    const artifacts = [
      {
        name: 'sia-iteration-report',
        content: `# SIA Iteration Report — ${intent.replace(/_/g, ' ')}\n\n` +
          `**Task:** ${task}\n\n` +
