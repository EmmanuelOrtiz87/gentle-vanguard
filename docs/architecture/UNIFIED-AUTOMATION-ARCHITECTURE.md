...
We have rules in `AGENTS.md`, `.clinerules`, `.cursorrules` but they're just text. The AI (me) reads
them but doesn't automatically obey them BEFORE every response.
...
**Option B: True Tool-Level Hook (Recommended)** Each tool needs to support a `pre_process` hook:
...
1. **Fix AI self-enforcement** (quick fix) - debug `pre-process-input.ps1` and make AI follow rules
   strictly
2. **Request tool vendors** (long-term) - ask OpenCode, Cline, Cursor to add `pre_process` hook
   support
3. **Hybrid** (recommended) - do both: fix current implementation + request proper tool support

**My recommendation**: Fix the script and make the rule ABSOLUTELY MANDATORY in all configs (which
we've done), then ensure AI follows it unconditionally.
