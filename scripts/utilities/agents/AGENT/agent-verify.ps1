...
if (Test-DomainEnabled 'skills') {
    ...
    if (Test-Path $AdPath) {
        try {
            ...
            $UserBase  = if ($env:CLAUDE_SKILLS_DIR) { $env:CLAUDE_SKILLS_DIR } elseif ($env:USERPROFILE) { Join-Path $env:USERPROFILE '.claude\skills' } else { "$Root\skills" }
...
"@ 2>&1
    ...
    $RoutingEvalScript = "$Root\build\protected\scripts\utilities\routing-quality-eval.ps1.enc"
    ...
    if ($Quick) {
        ...
        # Script is encrypted (.enc) — verify its existence, cannot execute
        if ($RoutingEvalScript -like "*.enc") {
            Add-Result "routing-language-matrix" "PASS" "Routing evaluator present (encrypted)" "tests"
        } else {
...
if (Test-DomainEnabled 'structure') {
    ...
    $RequiredScripts = @(
        ...
        "scripts/utilities/INSTALL/install-hooks.ps1"
    )
                ...
                if (-not (Test-Path $auditPath)) {
                    ...
                    Get-Content $auditPath -ErrorAction SilentlyContinue | ForEach-Object {
                        if (-not [string]::IsNullOrWhiteSpace($_)) {
                            try { $entries += ($_ | ConvertFrom-Json) } catch { Write-Debug "Exception caught: #!/usr/bin/env pwsh
# =============================================================================
#  agent-verify.ps1 - Self-Verification Tool for AI Agents
# =============================================================================
#
...
" }
                    ...
                    $recent = @($entries | Where-Object {
                        $ts = $null
                        if ($_.timestamp) {
                            try { $ts = [DateTimeOffset]::Parse([string]$_.timestamp).DateTime } catch { Write-Debug "Exception caught: #!/usr/bin/env pwsh
# =============================================================================
#  agent-verify.ps1 - Self-Verification Tool for AI Agents
# =============================================================================
#
