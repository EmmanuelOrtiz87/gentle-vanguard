...
const DEFAULT_CONFIG: LensesConfig = {
  ...
  reviewableExts: ['.ts', '.tsx', '.js', '.jsx', '.ps1', '.psm1', '.py', '.go', '.rs', '.json', '.yaml', '.yml', '.md'],
...
const RISK_PROFILES: Record<RiskLevel, RiskProfile> = {
  'low': { securityWeight: 0.1, maintainabilityWeight: 0.5, reliabilityWeight: 0.2, resilienceWeight: 0.2 },
  'medium': { securityWeight: 0.2, maintainabilityWeight: 0.3, reliabilityWeight: 0.3, resilienceWeight: 0.2 },
  'high': { securityWeight: 0.4, maintainabilityWeight: 0.15, reliabilityWeight: 0.25, resilienceWeight: 0.2 },
  'critical': { securityWeight: 0.5, maintainabilityWeight: 0.1, reliabilityWeight: 0.2, resilienceWeight: 0.2 },
};
...
function securityLens(content: string, filePath: string): LensFinding[] {
  ...
  // Hardcoded secrets
  const secretPattern = /(password|secret|api_key|apikey|token|credential|auth_token)\s*[:=]\s*["']{0,1}[^"',;\s]{8,}/i;
  for (let i = 0; i < lines.length; i++) {
    if (secretPattern.test(lines[i])) {
      findings.push({
        lens: 'security', severity: 'critical',
        title: 'Possible hardcoded secret',
        description: `Line ${i + 1}: potential secret/credential hardcoded in ${relPath}`,
        line: i + 1, snippet: lines[i].trim().slice(0, 100),
  ...
  for (let i = 0; i < lines.length; i++) {
    if (injectPattern.test(lines[i])) {
      findings.push({
        lens: 'security', severity: 'warning',
...
function maintainabilityLens(content: string, filePath: string): LensFinding[] {
  ...
  if (lines.length > 300) {
    findings.push({
      lens: 'maintainability', severity: 'warning',
      title: 'Large file',
  ...
  if (longLines > 10) {
    findings.push({
      lens: 'maintainability', severity: 'info',
...
function reliabilityLens(content: string, filePath: string): LensFinding[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/(fetch|axios\.get|http\.request|spawn|exec)\s*\(/.test(lines[i]) && !lines[i].includes('timeout') && !lines[i].includes('signal')) {
      findings.push({
        lens: 'reliability', severity: 'warning',
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/\.(map|filter|forEach|find)\s*\(/.test(lines[i]) && !lines[i].includes('?.') && !lines[i].includes('if')) {
      // Check if the array might be null
      const match = lines[i].match(/(\w+)\.(map|filter|forEach|find)\s*\(/);
      if (match && !content.includes(`if (!${match[1]})`) && !content.includes(`if (${match[1]} === null`)) {
        findings.push({
          lens: 'reliability', severity: 'info',
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/process\.exit\s*\(/.test(lines[i])) {
      findings.push({
        lens: 'reliability', severity: 'critical',
...
function resilienceLens(content: string, filePath: string): LensFinding[] {
  ...
  if (hasNetworkCall && !hasRetry) {
    findings.push({
      lens: 'resilience', severity: 'warning',
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/\bcatch\s*\(/.test(lines[i])) {
      ...
      while (j < Math.min(i + 5, lines.length)) {
        if (/fallback|default|return\s+\[\]|return\s+\{\}|return\s+null|alternative|cache/.test(lines[j])) {
          hasDegradation = true;
        }
        j++;
      }
...
export function reviewWithLens(
  ...
  try {
    ...
    switch (lens) {
      case 'security': return securityLens(content, filePath);
      case 'maintainability': return maintainabilityLens(content, filePath);
      case 'reliability': return reliabilityLens(content, filePath);
      case 'resilience': return resilienceLens(content, filePath);
    }
  } catch {
    return [];
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      case '--lens': lensName = (args[++i] ?? 'all') as LensName | 'all'; break;
      case '--target': target = args[++i] ?? '.'; break;
      case '--risk': risk = (args[++i] ?? 'medium') as RiskLevel; break;
  ...
  if (!quiet) {
    ...
    for (const review of reviews) {
      ...
      console.log(`  Findings: ${review.summary.total} (blocker: ${review.summary.blocker}, critical: ${review.summary.critical}, warning: ${review.summary.warning}, info: ${review.summary.info})`);
      for (const f of review.findings.slice(0, 10)) {
        console.log(`  [${f.severity}] ${f.title}`);
        if (f.line) console.log(`    Line ${f.line}: ${(f.snippet ?? '').slice(0, 80)}`);
        console.log(`    → ${f.recommendation}`);
      }
      if (review.findings.length > 10) console.log(`  ... and ${review.findings.length - 10} more findings`);
