...
1. **Pulls sales data** from QuickBooks or PayPal (transaction history, product/service revenue by
   date)
...
4. **Produces a 30-day brief** — ranked recommendations of what to push, what to hold, what offers
   to consider
...
2. If missing or "Unknown":
   - Ask: "I need your business category to pull the right seasonality benchmarks. What industry are
     you in?" (e.g., retail, services, SaaS)
...
- **QuickBooks:** Fetch invoice line items via `profit-loss-quickbooks-account` (pre-flight sets
  industry context)
- **PayPal:** Fetch merchant transactions via `list_transactions`. _Rate-limiting:_ If you hit rate
  limits, pause 30 seconds and retry once. If still blocked, gracefully offer: "PayPal is
  rate-limited. Would you like to switch to QuickBooks or Square instead, or I can continue with
  historical data I already pulled?"
- **Square:** Requires location ID first. Call
  `make_api_request(service="locations", method="list")` to discover available locations, then fetch
  orders for each location. _Future enhancement:_ Square integration is stubbed; full path
  documented in `reference/square-integration.md`.

**Fallback:** If <3 months of data, use industry seasonality benchmarks for the SMB's category
(e.g., retail, services, e-commerce)
...
- **Industry benchmarks:** For categories without strong user data (e.g., "Q1 is strong for tax
  services")
...
- **Executive summary** (1–2 sentences: "Your best sellers are X and Y. Seasonal shift to Z is
  starting.")
- **Push hard** (Top 2–3 products + recommended content angle, e.g., "Case study on ROI", "How-to
  video")
...
Iterate if needed; once approved, return the final brief as structured JSON (ready for downstream
tools).
