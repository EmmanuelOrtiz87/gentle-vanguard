...
- **Dashboard & Metrics**:
  - `scripts/metrics/dashboard-render.ps1` — genera HTML dashboard con charts canvas
  - `scripts/metrics/metrics-server.ps1` — sirve dashboard en localhost:8090 con API REST
  - `scripts/metrics/live-feed.ps1` — collector periódico de métricas en segundo plano
  - `scripts/metrics/collector.ps1` — recolecta métricas a .runtime/metrics/*.json
  - `scripts/utilities/live-feed-manager.ps1` — orquesta start/stop/status de background processes
  - `scripts/utilities/background-watchdog.ps1` — monitorea y auto-reinicia procesos caídos
  - `scripts/utilities/export-dashboard-pdf.ps1` — exporta dashboard a PDF via headless browser
  - `scripts/utilities/TELEMETRY-METRICS/sla-dashboard-generator.ps1` — SLA dashboard con uptime, incidentes, compliance
  - `scripts/utilities/TELEMETRY-METRICS/generate-dashboard.ps1` — dashboard completo con ROI, costos, benchmarks, sesiones
