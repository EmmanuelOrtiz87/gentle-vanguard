...
function check(name: string, pass: boolean, message: string, severity: 'PASS' | 'WARN' | 'FAIL' = 'PASS'): void {
...
function checkOpenCodeSubagents(): void {
  ...
  try {
    ...
    check('opencode.json parsable', true, `${agentNames.length} agents (${subagents.length} subagents)`);
...
function checkAutoDelegation(): void {
  ...
  try {
    ...
    if (missing.length > 0) {
      check('auto-delegation references', false, 
        `${missing.length} agents not in opencode.json: ${missing.join(', ')}`, 'WARN');
    } else {
      check('auto-delegation references', true, `${availableNames.length} agents match opencode.json`);
    }
    
    check('auto-delegation _meta', hasMeta, hasMeta ? '_meta section present' : 'Missing _meta section', 'WARN');
  } catch (err) {
...
function checkDeprecatedScripts(): void {
  ...
  if (found.length > 0) {
    check('Deprecated bypass scripts', false, 
      `${found.length} non-deprecated scripts found: ${found.join(', ')}`, 'WARN');
  } else {
...
function checkSessionAutostart(): void {
  ...
  check('session-autostart.cmd references TS', referencesTS, 
...
function checkModelSwitch(): void {
  ...
  try {
    ...
    if (hasModelCurrent && hasModelSwitch && hasModelList && hasModelHelp) {
      check('model-switch npm scripts', true, 'model:current, model:switch, model:list, model:help');
    } else {
      const missing = [];
    ...
    check('model-switch.ts exists', existsSync(scriptPath), 
      existsSync(scriptPath) ? 'Script found' : 'Script not found', 'WARN');
    ...
    if (active?.model) {
      ...
      check('Active model persisted', false, 'No model:switch executed yet (run npm run model:switch <model>)', 'WARN');
    }
  } catch (err) {
...
function checkProviderConfig(): void {
  ...
  try {
    ...
    if (providerNames.length > 0) {
      check('Provider configs', true, `${providerNames.length} providers: ${providerNames.join(', ')}`);
      
      // Check openrouter has models
      if (providers.openrouter) {
        const modelCount = Object.keys(providers.openrouter.models || {}).length;
        check('OpenRouter models', modelCount > 0, `${modelCount} models configured`, modelCount > 0 ? 'PASS' : 'WARN');
      }
    } else {
