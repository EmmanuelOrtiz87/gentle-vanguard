...
> **29 agents · 132 skills · CLI `gentle-vanguard` · auto-delegation · Engram memory · 7D pre-commit
> hooks**
...
**✅ Output:** `main.go`, `go.mod`, OpenAPI `docs/sdd/user-api.yaml`, Dockerfile, CI workflow,
passing tests
...
**✅ Output:** `AuthContext.tsx`, `useAuth.ts`, `LoginPage.tsx`, `SignupPage.tsx`, `auth.e2e.ts`,
...
**🔄 Auto:** All 14 quality gates, secret scanning, dependency audit, script validation, report
generation  
**👤 Manual:** Review findings for false positives, prioritize fixes, apply patches  
**✅ Output:** `docs/audits/security-audit-*.md` with `CRITICAL/HIGH/MEDIUM/LOW` breakdown, CVE
...
**✅ Output:** Git tag `v2.7.0`, GitHub Release, `CHANGELOG.md` updated,
`docs/operations/release-v2.7.0.md`
...
**✅ Output:** `docs/premortems/payment-service-v2-premortem.md` with 10+ scenarios, risk matrix,
action items
...
**📊 Dashboard sections:** | Section | Data | |---------|------| | Session activity | Sessions
started, avg duration, most active agents | | Token usage | Total consumed, trend vs previous week,
budget health | | Quality | Judgment-day pass rate, open findings, coverage delta | | Git health |
Branches created, PRs merged, merge conflict count |
...
**Trigger:** "fastapi", "fast api", "uvicorn" **Stack:** Python 3.11+, Pydantic v2, SQLAlchemy 2.0
...
**✅ Output:** `skills/fastapi-patterns/SKILL.md`, updated `config/auto-delegation.json`, skill
registry entry
...
**✅ Output:** `docs/sdd/notification-service-v1/`, `internal/handler/`, `internal/provider/`,
`docker-compose.yml`, `README.md`, passing test suite, CI pipeline
...
> **Prerequisite:** Always start with `gv start-session` (or
> `.\scripts\utilities\session-autostart.cmd`) to initialize Engram memory and routing context.
