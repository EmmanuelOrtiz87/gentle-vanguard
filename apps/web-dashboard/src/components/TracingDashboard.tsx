...
import { Activity, Clock, AlertCircle, GitBranch, Search, ChevronDown, ChevronUp, ThumbsUp, ThumbsDown } from 'lucide-react';
...
function TraceWaterfall({ trace, allTraces }: { trace: Trace; allTraces: Trace[] }) {
  ...
  const barColor = trace.status === 'error' ? 'bg-red-500' : trace.status === 'running' ? 'bg-blue-400 animate-pulse' : 'bg-emerald-500';
    ...
    <div className="select-none">
      <div className="flex items-center gap-2 py-1.5 px-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded group">
        ...
        <div className="flex items-center gap-1.5 min-w-[180px]">
          <span className={`w-1.5 h-1.5 rounded-full ${trace.status === 'completed' ? 'bg-green-500' : trace.status === 'running' ? 'bg-blue-500' : 'bg-red-500'}`} />
          <span className="text-xs font-mono text-gray-700 dark:text-gray-300 truncate">{trace.name}</span>
        </div>
        <div className="flex-1 h-4 bg-gray-100 dark:bg-gray-700 rounded overflow-hidden min-w-[100px]">
          <div className={`h-full ${barColor} rounded transition-all duration-300`} style={{ width: `${barWidth}%` }} />
        </div>
        <span className="text-xs text-gray-500 dark:text-gray-400 w-16 text-right tabular-nums">{trace.duration ? `${trace.duration}ms` : '-'}</span>
        <span className="text-xs text-gray-400 dark:text-gray-500 w-12 text-right">{trace.attributes.model || ''}</span>
      </div>
...
function TraceDetail({ trace, allTraces }: { trace: Trace; allTraces: Trace[] }) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
              trace.status === 'completed' ? 'text-green-700 bg-green-50 dark:text-green-300 dark:bg-green-900/20' :
              trace.status === 'error' ? 'text-red-700 bg-red-50 dark:text-red-300 dark:bg-red-900/20' :
      ...
      {children.length > 0 && (
        <div className="card">
          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Child Spans ({children.length})</h4>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  <th className="text-left py-2 px-3 text-xs font-medium text-gray-500">Name</th>
                  <th className="text-right py-2 px-3 text-xs font-medium text-gray-500">Duration</th>
              ...
              <tbody>
                {children.map(c => (
                  <tr key={c.spanId} className="border-b border-gray-100 dark:border-gray-800">
                    <td className="py-2 px-3 text-gray-800 dark:text-gray-200 font-mono text-xs">{c.name}</td>
                    <td className="py-2 px-3 text-right text-gray-600 dark:text-gray-400">{c.duration}ms</td>
                    <td className="py-2 px-3 text-right text-gray-600 dark:text-gray-400">{c.attributes.inputTokens}</td>
                    <td className="py-2 px-3 text-right text-gray-600 dark:text-gray-400">{c.attributes.outputTokens}</td>
                    <td className="py-2 px-3 text-right text-gray-600 dark:text-gray-400">${parseFloat(c.attributes.cost || '0').toFixed(4)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function FeedbackButtons({ traceId, spanId }: { traceId: string; spanId: string }) {
  ...
  return (
    <div className="flex items-center gap-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
      <button onClick={() => sendFeedback('up')} className={`p-1 rounded hover:bg-green-100 dark:hover:bg-green-900/30 ${sent === 'up' ? 'text-green-500' : 'text-gray-400'}`}>
        <ThumbsUp className="w-3 h-3" />
      </button>
      <button onClick={() => sendFeedback('down')} className={`p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/30 ${sent === 'down' ? 'text-red-500' : 'text-gray-400'}`}>
...
export function TracingDashboard() {
  ...
  const [stats, setStats] = useState<TraceStats>({ totalTraces: 0, avgDuration: 0, errorRate: 0, activeSpans: 0 });
  ...
  const filteredRoots = rootTraces.filter(t => {
    if (search && !t.name.toLowerCase().includes(search.toLowerCase()) && !t.traceId.toLowerCase().includes(search.toLowerCase())) return false;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card">
          <div className="flex items-center justify-between">
            <div><p className="metric-label">Total Traces</p><p className="metric-value">{stats.totalTraces}</p></div>
            <Activity className="w-8 h-8 text-blue-500" />
          </div>
        </div>
        <div className="card">
          <div className="flex items-center justify-between">
            <div><p className="metric-label">Avg Duration</p><p className="metric-value">{stats.avgDuration}ms</p></div>
            <Clock className="w-8 h-8 text-yellow-500" />
          </div>
        </div>
        <div className="card">
          <div className="flex items-center justify-between">
            <div><p className="metric-label">Error Rate</p><p className="metric-value">{(stats.errorRate * 100).toFixed(1)}%</p></div>
            <AlertCircle className="w-8 h-8 text-red-500" />
          </div>
        </div>
        <div className="card">
          <div className="flex items-center justify-between">
            <div><p className="metric-label">Active Spans</p><p className="metric-value">{stats.activeSpans}</p></div>
      ...
      <div className="card">
        ...
        {traces.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            ...
            <p className="text-xs text-gray-500 mt-1">Traces are read from .session/context-log/*/.state.json</p>
          </div>
        ) : (
          <>
            {/* Waterfall header */}
      ...
      <div className="card">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {traces.map(t => (
                <tr key={t.spanId} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer" onClick={() => setSelectedTrace(t)}>
                  <td className="py-2 px-4 text-sm font-mono text-gray-600 dark:text-gray-400">{t.spanId.substring(0, 16)}</td>
                  <td className="py-2 px-4 text-sm text-gray-900 dark:text-white">{t.name}</td>
                  <td className="py-2 px-4">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                      t.status === 'completed' ? 'text-green-700 bg-green-50 dark:text-green-300 dark:bg-green-900/20' :
                      t.status === 'running' ? 'text-blue-700 bg-blue-50 dark:text-blue-300 dark:bg-blue-900/20' :
                      'text-red-700 bg-red-50 dark:text-red-300 dark:bg-red-900/20'
                    }`}>{t.status}</span>
                  </td>
                  <td className="py-2 px-4 text-sm text-right text-gray-600 dark:text-gray-400 tabular-nums">{t.duration ? `${t.duration}ms` : '-'}</td>
                  <td className="py-2 px-4 text-sm text-gray-600 dark:text-gray-400">{t.attributes.model || '-'}</td>
                  <td className="py-2 px-4 text-sm text-right text-gray-600 dark:text-gray-400 tabular-nums">{t.attributes.inputTokens}</td>
                  <td className="py-2 px-4 text-sm text-right text-gray-600 dark:text-gray-400 tabular-nums">{t.attributes.outputTokens}</td>
                  <td className="py-2 px-4 text-sm text-right text-gray-600 dark:text-gray-400 tabular-nums">${parseFloat(t.attributes.cost || '0').toFixed(4)}</td>
                </tr>
              ))}
            </tbody>
          </table>
