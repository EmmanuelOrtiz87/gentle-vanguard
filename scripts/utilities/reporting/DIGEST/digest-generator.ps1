...
if (Test-Path $sessionFile) {
    try { $sessionId = (Get-Content $sessionFile -Raw | ConvertFrom-Json -ErrorAction Stop).sessionId } catch { Write-Debug "Exception caught: <#
.SYNOPSIS
    Generates proactive daily/status digests summarizing system activity.
...
if (Test-Path $summaryFile) {
    try {
        ...
        $platform = $s.platform
        $tool = $s.tool
    } catch { Write-Debug "Exception caught: <#
.SYNOPSIS
    Generates proactive daily/status digests summarizing system activity.
