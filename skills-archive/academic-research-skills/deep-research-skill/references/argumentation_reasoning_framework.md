...
A cognitive framework for evaluating the strength and validity of research arguments. Use this to
**think about** argument quality, not just check boxes.
...
**Judgment heuristic**: If you can't identify the Warrant, the argument is likely weak regardless of
how much Data is presented. Data without Warrant is just information.
...
**Judgment heuristic**: Most social science papers satisfy 3-5 criteria. Fewer than 3 = causal claim
is unsupported. Only #4 (temporality) is strictly required; the rest are cumulative evidence.
...
2. Evaluate each on: **explanatory scope** (how much it explains), **simplicity** (fewer ad-hoc
   assumptions), **fit** (consistency with known facts), **predictive power** (does it predict new
   observations?)
3. The best explanation is the one that scores highest across all four — not the one that fits the
   author's hypothesis

**Judgment heuristic**: If the paper only considers one explanation, that's confirmation bias
regardless of how well-argued it is. At minimum, the Discussion section should address the two
...
**Judgment heuristic**: If a paper uses "Established" language for "Preliminary" findings, that's
overclaiming — one of the most common quality issues in academic writing.
