...
function Get-TokenAutopilotPolicy {
    ...
    try { $cfg = Get-Content $configPath -Raw -Encoding UTF8 | ConvertFrom-Json; if ($cfg.tokenAutopilot) { foreach ($k in $defaults.Keys) { if ($null -ne $cfg.tokenAutopilot.$k) { $defaults[$k] = $cfg.tokenAutopilot.$k } } } } catch { Write-Debug "Exception caught: function Invoke-TokenBudgetGuard {
    param([string]$Task, [string]$Risk = 'medium', [int]$EstimatedChars = 0, [int]$ActualPromptTokens = 0, [int]$ActualCompletionTokens = 0)
    $guardScript = Join-Path $global:scriptDir '..\TELEMETRY-METRICS\token-budget-guard.ps1'
    if (-not (Test-Path $guardScript)) { return }
    $guardArgs = @{ Mode = 'check'; Task = $Task; Risk = $Risk; Record = $true; AsJson = $true; Quiet = $true }
...
function Get-TokenAutopilotState { param([string]$StatePath); if (Test-Path $StatePath) { try { return Get-Content $StatePath -Raw -Encoding UTF8 | ConvertFrom-Json } catch { Write-Debug "Exception caught: function Invoke-TokenBudgetGuard {
    param([string]$Task, [string]$Risk = 'medium', [int]$EstimatedChars = 0, [int]$ActualPromptTokens = 0, [int]$ActualCompletionTokens = 0)
    $guardScript = Join-Path $global:scriptDir '..\TELEMETRY-METRICS\token-budget-guard.ps1'
    if (-not (Test-Path $guardScript)) { return }
    $guardArgs = @{ Mode = 'check'; Task = $Task; Risk = $Risk; Record = $true; AsJson = $true; Quiet = $true }
...
" } }; [pscustomobject]@{consecutiveAlerts=0;lastStatus='PASS';lastTask='';lastAppliedChatLevel='';lastAppliedAt=''} }
...
function Invoke-ContextEfficiencyLiveAssist {
    ...
    if ($livePolicy.AutoRunOnStartSessionWhenRed -and $normalized -eq 'start-session' -and ($metrics.HealthStatus -like 'RED*')) {
        ...
        $marker = Join-Path $global:repoRoot '.session\.compact-marker'
        $alreadyRun = $false
        if (Test-Path $marker) { try { $last = Get-Content $marker -Raw; if ($last -match '\d') { $t = [datetime]::Parse($last,[cultureinfo]::InvariantCulture,[System.Globalization.DateTimeStyles]::AssumeUniversal); if ((Get-Date).ToUniversalTime() - $t -lt [timespan]::FromMinutes(60)) { $alreadyRun = $true } } } catch { Write-Debug "Exception caught: function Invoke-TokenBudgetGuard {
    param([string]$Task, [string]$Risk = 'medium', [int]$EstimatedChars = 0, [int]$ActualPromptTokens = 0, [int]$ActualCompletionTokens = 0)
    $guardScript = Join-Path $global:scriptDir '..\TELEMETRY-METRICS\token-budget-guard.ps1'
    if (-not (Test-Path $guardScript)) { return }
    $guardArgs = @{ Mode = 'check'; Task = $Task; Risk = $Risk; Record = $true; AsJson = $true; Quiet = $true }
