...
const AGENT_CONFIG: DomainAgentConfig = {
  ...
  execute: (task, context, prompt) => {
    ...
    const intent =
      normalized.includes('contract') || normalized.includes('agreement') || normalized.includes('dpa') ? 'CONTRACT_REVIEW' :
      normalized.includes('gdpr') || normalized.includes('compliance') || normalized.includes('privacy') || normalized.includes('hipaa') || normalized.includes('soc') ? 'COMPLIANCE' :
      normalized.includes('incident') || normalized.includes('breach') ? 'INCIDENT_RESPONSE' :
      normalized.includes('memo') || normalized.includes('legal memo') ? 'LEGAL_MEMO' :
      normalized.includes('escalat') ? 'ESCALATION' :
      'GENERAL';
    ...
    const checklist = [
      ...
      intent === 'CONTRACT_REVIEW' ? 'Contract: parties, jurisdiction, governing law, term, consideration' : 'Compliance records: privacy policy, ToS, DPAs, security policies'
    ];

    const flags: DomainOutput['flags'] = [
      { severity: 'critical', message: 'Legal output is advisory, not legal advice. Escalate to qualified counsel before acting.', advisory: true }
    ];
    if (intent === 'INCIDENT_RESPONSE') {
      flags.push({ severity: 'critical', message: 'Breach response: Contain → Assess → Notify → Document → Remediate → Review. Check regulatory timelines.', advisory: true });
    }
    if (intent === 'CONTRACT_REVIEW') {
      flags.push({ severity: 'warn', message: 'Review key clauses: indemnification, limitation of liability, termination, IP rights, confidentiality, non-compete.' });
    }

    const artifacts = [
      {
        name: 'legal-analysis',
        content: `# Legal Analysis — ${intent.replace(/_/g, ' ')}\n\n` +
          `**Task:** ${task}\n\n` +
          `## Critical Rules\n\n` +
    ...
    return {
      ...
      checklist,
      artifacts,
      evidence: ['LEGAL.md domain prompt loaded', 'Regulatory frameworks + breach timelines applied'],
      flags
    };
  }
};
