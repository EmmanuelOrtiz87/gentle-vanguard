...
- Session artifacts LOCAL-ONLY: closure reports, delivery checklists, context packs → `.local/session-artifacts/`
...
- Session state tracked in `.session/` with `session-state.json`, `token-usage.json`, `activity-log.json`
...
- Levels: README (overview) → Getting Started (install) → User Guide (features) → API Reference → Architecture
...
- Command: `skill-factory.ps1 -Name <name> -Description <desc> -Agent <agent> -Triggers "t1,t2,t3" [-Register]`
