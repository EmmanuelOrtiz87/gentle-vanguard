...
interface OverItem { skill: string; dir: string; lines: number; tokens: number; sizeKB: number; issues: string; }

function scanDir(rootDir: string, label: string, maxLines: number, maxTokens: number): { over: OverItem[]; totalFiles: number } {
...
function main(): void {
  ...
  const maxTokens = args.includes('--max-tokens') ? parseInt(args[args.indexOf('--max-tokens') + 1], 10) : 1000;
  const maxLines = args.includes('--max-lines') ? parseInt(args[args.indexOf('--max-lines') + 1], 10) : 150;

  const allOver: OverItem[] = [];
  let allTotal = 0;

  // Always scan .opencode/skills/ (built-in opencode skills)
  const dotResult = scanDir(join(ROOT, '.opencode', 'skills'), '.opencode/skills', maxLines, maxTokens);
  ...
  if (allOver.length === 0) {
    console.log(`[OK] All ${allTotal} skills within limits (${maxLines} lines / ${maxTokens} tokens)`);
    process.exit(0);
  }
  ...
  console.log(`[WARN] ${allOver.length} skills exceed limits (max ${maxTokens} tokens / ${maxLines} lines) — ${summary}:`);
  console.table(allOver.map(o => ({ Dir: o.dir, Skill: o.skill, Tokens: o.tokens, Lines: o.lines, 'Size(KB)': o.sizeKB, Issues: o.issues })));
