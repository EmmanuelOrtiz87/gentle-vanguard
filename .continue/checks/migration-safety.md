...
- Column type narrowing (e.g., `TEXT` to `VARCHAR(50)`, `BIGINT` to `INT`) without backfill -- add
  backfill/guard
- `NOT NULL` constraint added to existing column without `DEFAULT` value -- add default or backfill
- Renaming a column/table referenced by code without updating references in same PR -- update
  references
