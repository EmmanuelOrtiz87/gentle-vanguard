...
function main(): void {
  ...
  const mode = (args.includes('--mode') ? args[args.indexOf('--mode') + 1] : 'status') as DigestMode;
  ...
  if (existsSync(sessionFile)) {
    try { sessionId = JSON.parse(readFileSync(sessionFile, 'utf-8')).sessionId || ''; } catch { /* ignore */ }
  }

  let platform = '';
  let tool = '';
  if (existsSync(summaryFile)) {
    try { const s = JSON.parse(readFileSync(summaryFile, 'utf-8')); platform = s.platform || ''; tool = s.tool || ''; } catch { /* ignore */ }
  }
  ...
  let feedbackEntries: Array<{ timestamp?: string; rate?: number; action?: string; comment?: string }> = [];
  if (existsSync(feedbackFile)) {
    try {
      const raw = readFileSync(feedbackFile, 'utf-8').split('\n').filter(l => l.trim());
      feedbackEntries = raw.map(l => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
    } catch { /* ignore */ }
  }
  const weekAgo = Date.now() - 7 * 86400000;
  const recentFeedback = feedbackEntries.filter(e => e.timestamp && new Date(e.timestamp).getTime() >= weekAgo);
  const rated = recentFeedback.filter(e => (e.rate ?? 0) > 0);
  const avgRating = rated.length > 0 ? Math.round(rated.reduce((s, e) => s + (e.rate ?? 0), 0) / rated.length * 10) / 10 : 0;
  ...
  if (asJson) {
    console.log(JSON.stringify({
      date: today, mode, sessionId, platform, tool, branch,
      commits24h: commitCount, health: healthStatus,
  ...
  const lines: string[] = [];
  lines.push(`# Digest: ${today}`, `**Mode**: ${mode}  |  **Session**: ${sessionId}  |  **Platform**: ${platform}  |  **Tool**: ${tool}`);
  if (branch) lines.push(`**Branch**: ${branch}  |  **Commits (24h)**: ${commitCount}`);
  lines.push('', '## Health', `- System: ${{ ok: 'OK', warn: 'Warning', unknown: 'Unknown' }[healthStatus] || 'Unknown'}`);
  if (tokenSpend) lines.push(`- Token spend: ${tokenSpend}`);
  lines.push('');

  if (recentFeedback.length > 0) {
    lines.push(`## Feedback (7 days)`, `- Entries: ${recentFeedback.length}, Avg rating: ${avgRating}/5`);
  ...
  if (pendingProposals > 0) {
    lines.push(`## Pending Proposals`, `- ${pendingProposals} pending improvement(s)`, `- Run \`gv learning apply\` or \`gv digest\` to review`, '');
  }

  lines.push('## Next Steps', '- Review digest: `gv digest`', '- Submit feedback: `gv feedback rate 4 -Action <action> -Comment "..."`', '- Apply learnings: `gv learning auto`', '-');
