...
function getNudgeSequence(): number {
  ...
  let seq = 1;
  if (existsSync(seqFile)) { try { seq = parseInt(readFileSync(seqFile, 'utf-8').trim(), 10) + 1; } catch { /* */ } }
  writeFileSync(seqFile, String(seq), 'utf-8');
  return seq;
}

function getPreviousFailures(skillName: string): number {
  const files = existsSync(nudgeDir) ? readdirSync(nudgeDir).filter(f => f.endsWith('.json') && f !== '.sequence') : [];
  let total = 0;
  for (const f of files) {
    try { const n: Nudge = JSON.parse(readFileSync(join(nudgeDir, f), 'utf-8')); if (n.skillName === skillName && n.issueType === 'failure_pattern') total++; } catch { /* */ }
  }
  return total;
}

function writeNudge(skillName: string, issueType: string, evidence: string, fixPattern: string, urgent: boolean): Nudge {
  ensureDir(nudgeDir);
  const seq = getNudgeSequence();
  const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const nudgeId = `nudge-${date}-${String(seq).padStart(3, '0')}`;
  const nudge: Nudge = { nudgeId, date: new Date().toISOString(), skillName, trigger: 'auto', issueType, evidence, fixPattern, urgent, applied: false };
...
function generateNudges(): void {
  ...
  const usageFiles = existsSync(usageDir) ? readdirSync(usageDir).filter(f => f.endsWith('.json')) : [];
  if (usageFiles.length === 0) { console.log('[INFO] No usage data found'); return; }
  ...
  for (const f of usageFiles) {
    try {
      ...
      if (lastUse && lastUse.toISOString().slice(0, 10) === today && m.lastOutcome === 'failure') {
        if (!sessionFailures[m.skillName]) sessionFailures[m.skillName] = { failures: 0, hasFailures: false };
  ...
  for (const m of allMetrics) {
    ...
    if (sf.hasFailures && m.failureCount > 0) {
      ...
      if (existsSync(skillMdPath)) hasSection = /## Known Issues|## Failure Patterns/.test(readFileSync(skillMdPath, 'utf-8'));
      else if (existsSync(altPath)) hasSection = /## Known Issues|## Failure Patterns/.test(readFileSync(altPath, 'utf-8'));
      if (hasSection) { console.log(`[NUDGE] Skip ${m.skillName} — Known Issues section already exists`); continue; }

      const evidence = `${m.failureCount} total failures, ${sf.failures} in current session`;
      const errorType = m.failurePatterns[0]?.errorType || '';
      const fixPattern = ({ timeout: 'Add timeout configuration and retry logic', syntax: 'Add syntax validation before execution', logic: 'Review conditional logic and edge cases', 'missing_dependency': 'Add dependency check at start of skill execution' })[errorType] || 'Review failure patterns and update SKILL.md with known issues';
    ...
    if (m.useCount >= 10 && m.successRate < 0.7) {
      ...
      writeNudge(m.skillName, 'declining_rate', evidence, 'Review skill triggers and core rules for accuracy', isUrgent);
      nudgeCount++;
    }
    ...
    if (m.useCount <= 1 && !m.lastUsedAt) {
      ...
      if (existsSync(metricFile)) {
        try {
          ...
          if (ageDays > 7) {
            writeNudge(m.skillName, 'underused', `Never used in ${Math.round(ageDays)} days`, 'Check if skill triggers are too narrow or skill is deprecated', false);
            nudgeCount++;
          }
        } catch { /* */ }
      }
...
function showReport(): void {
  const nudgeFiles = existsSync(nudgeDir) ? readdirSync(nudgeDir).filter(f => f.endsWith('.json') && f !== '.sequence').sort().reverse() : [];
  if (nudgeFiles.length === 0) { console.log('[INFO] No nudges found'); return; }

  console.log(`\n=== Skill Nudge Report ===`);
  let urgentCount = 0, pendingCount = 0;
  for (const f of nudgeFiles) {
    try {
      ...
      console.log(`${n.nudgeId} | ${n.skillName} | ${n.issueType} | ${n.urgent ? 'yes' : 'no'} | ${n.applied ? 'yes' : 'no'}`);
    } catch { /* */ }
  }
