---
name: ai-provenance
description: Inspect and manage AI provenance marks (invisible Unicode, C2PA/Content Credentials, EXIF/XMP metadata, SynthID-class watermarks). DEFAULT is inspection only; removal is strictly on-demand with explicit user request.
triggers:
  - provenance
  - watermark detect
  - c2pa
...
name: ai-provenance
description: >
  Inspect and manage AI provenance marks (invisible Unicode, C2PA/Content Credentials,
  EXIF/XMP metadata, SynthID-class watermarks) on text and files. DEFAULT MODE is
  INSPECTION ONLY (detect, report, verify). REMOVAL is STRICTLY ON-DEMAND: only when
  the user explicitly and unambiguously requests stripping/removing provenance marks
  from content they own — never proactively, never in normal behavior. Use for
  provenance verification, AI-content attribution checks, and (only on explicit
...
Gestión de marcas de proveniencia AI para **texto** (Unicode + watermark estadístico) y **archivos** (C2PA / metadatos AI en contenedores comunes).

## ⚖️ Política de uso (CRÍTICA)

| Modo | Comportamiento | Cuándo |
| --- | --- | --- |
| **INSPECCIÓN** (default) | Detectar, reportar y verificar marcas de proveniencia | Comportamiento normal del stack, sin solicitud previa |
| **REMOCIÓN** (on-demand) | Limpiar C2PA/Unicode/metadatos | **SOLO** cuando el usuario lo solicita explícita e inequívocamente para contenido propio |

- El stack **nunca** remueve marcas de proveniencia en comportamiento normal.
- La remoción se activa únicamente ante una petición explícita del usuario (p. ej. "quita la marca de agua de este archivo mío").
- Ver `references/ethics.md` antes de cualquier remoción; si el fin es fraude académico o encubrimiento ilícito, advertir y no ejecutar.
