...
Quick-reference for switching between 5 citation formats. Used by `citation_compliance_agent` and
`formatter_agent`.
...
**Key rules**: No year in in-text, page numbers always, containers model, no DOI in basic format
(include if online), title case for all titles.
...
**Key rules**: Numbered in order of appearance, no spaces in initials, abbreviated journal titles
(per NLM), semicolon before volume, colon before pages.
...
**Same author with both Chinese and English works**: List separately — Chinese works go in the
Chinese section, English works go in the English section.
