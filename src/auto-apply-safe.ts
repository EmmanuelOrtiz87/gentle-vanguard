...
function getLogger(quiet: boolean) {
  return (msg: string) => { if (!quiet) console.log(`[AUTO-APPLY] ${msg}`); };
}
...
function getPendingBudgetActions(): SafeAction[] {
  ...
  for (const file of files.slice(-3)) { // Last 3 reports
    try {
...
function getPendingDeprecationActions(): SafeAction[] {
  ...
  for (const file of files.slice(-1)) { // Latest report
    try {
...
function getPendingNormActions(): SafeAction[] {
  const actions: SafeAction[] = [];
  const norms = loadJson<Array<{ id: string; description: string; confidence: number; status: string; occurrences: number }>>(NORMS_DB, []);
...
function getPendingTriggerActions(): SafeAction[] {
  ...
  const files = readdirSync(TRIGGER_DIR).filter(f => f.startsWith('trigger-') && f.endsWith('.json'));
  for (const file of files) {
    try {
      ...
      switch (trigger.type) {
        ...
        case 'norm-promotion':
          action = {
            ...
            description: trigger.description || `Auto-promote ${trigger.promotedCount || 0} norm(s) (trigger)`,
            confidence: 0.85,
            riskLevel: 'medium',
            autoApply: true,
          };
      ...
      try {
        writeFileSync(join(TRIGGER_DIR, file), JSON.stringify({ ...trigger, processed: true, processedAt: new Date().toISOString() }, null, 2), 'utf-8');
      } catch { /* non-fatal */ }
    } catch { /* skip unparseable trigger */ }
  }
...
function applyDeprecation(action: SafeAction): { success: boolean; error?: string } {
  ...
  writeFileSync(depFile, JSON.stringify({
    actionId: action.id,
...
function checkDegradation(actionLog: ActionLog[]): ActionLog | null {
  ...
  for (const entry of actionLog.slice().reverse()) {
    ...
    const qualityDrop = before.qualityScore && after.qualityScore
...
function main(): void {
  ...
  if (args.action === 'report') {
    ...
    console.log(JSON.stringify({
      ...
      successRate: actionLog.length > 0
        ? Math.round((actionLog.filter(e => e.result === 'success').length / actionLog.length) * 100)
        : 0,
