...
interface DelegationConfig {
  ...
  routingBindings?: {
    tiers: { tier1_direct: { confidenceMin: number }; tier2_confirm: { confidenceMin: number }; tier3_clarify: { confidenceMin: number } };
  };
}
...
function tokenize(text: string): string[] {
  ...
  return cleaned.split(/[\s-]+/).filter(t => t.length >= 2 && t.length <= 40 && !stopWordsSet.has(t));
}
...
function parseArgs(): MlrRouterArgs {
  ...
  return {
    ...
    embeddingsPath: extractArg(raw, '--embeddings-path') || join(projectRoot, '.atl', 'skill-embeddings.json'),
    delegationConfigPath: extractArg(raw, '--delegation-config-path') || join(projectRoot, 'config', 'auto-delegation.json'),
    registryPath: extractArg(raw, '--registry-path') || join(projectRoot, '.atl', 'skill-registry.md'),
...
function loadEmbeddings(path: string): Embeddings {
  ...
  const skills: EmbeddingEntry[] = (json.skills || []).map((s: Record<string, unknown>) => ({
    ...
    charNgrams: ((s.charNgrams as string[]) || []).reduce((acc: Record<string, boolean>, g: string) => { acc[g] = true; return acc; }, {}),
  }));

  return {
    metadata: { totalSkills: skills.length, vocabularySize: Object.keys(vocabulary).length },
    vocabulary, idf, skills, sourcePath: path,
  };
}
...
function computeQueryVector(query: string, vocabulary: Record<string, number>, idf: Record<string, number>): QueryVectorInfo {
...
function computeCosineSimilarity(queryVector: Record<string, number>, skillVector: Record<string, number>): CosineResult {
  let dot = 0;
  let shared = 0;
...
function computeJaccardSimilarity(queryNgrams: Record<string, boolean>, skillNgrams: Record<string, boolean>): number {
...
function invokeSkillMatching(queryText: string, embeddings: Embeddings, delegationConfig: DelegationConfig | null): MatchResult[] {
...
function main(): void {
  ...
  console.log(`Loaded ${embeddings.metadata.totalSkills} skills with ${embeddings.metadata.vocabularySize} vocabulary terms`);
  ...
  console.log(`${'Rank'.padEnd(5)} ${'Skill'.padEnd(30)} ${'Agent'.padEnd(8)} ${'Score'.padEnd(8)} ${'MatchType'.padEnd(18)}`);
  console.log('-'.repeat(70));

  let rank = 1;
  for (const m of topMatches) {
    const scorePct = Math.round(m.score * 100);
    console.log(`${String(rank).padEnd(5)} ${m.skill.padEnd(30)} ${m.agent.padEnd(8)} ${String(scorePct).padEnd(3)}%  ${m.matchType.padEnd(18)}`);
    rank++;
  }
