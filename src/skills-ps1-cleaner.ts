...
const REPLACEMENTS = [
  ...
  { 
    pattern: /```powershell\s*\.\\scripts\\utilities\\invoke-cloud-agent\.ps1\s+-Provider\s+(\w+)\s+-Command\s+"([^"]+)"\s*```/g,
    replacement: '```bash\nnpx tsx src/invoke-cloud-agent.ts --provider $1 --command "$2"\n```'
  },
  { 
    pattern: /```powershell\s*\.\\scripts\\utilities\\invoke-cloud-agent\.ps1\s+-Provider\s+(\w+)\s+-Script\s+"([^"]+)"\s*```/g,
    replacement: '```bash\nnpx tsx src/invoke-cloud-agent.ts --provider $1 --script "$2"\n```'
  },
  { 
    pattern: /```powershell\s*\.\\scripts\\utilities\\invoke-cloud-agent\.ps1\s+-Interactive\s*```/g,
    replacement: '```bash\nnpx tsx src/invoke-cloud-agent.ts --interactive\n```'
  },
  { 
    pattern: /```powershell\s*\.\\scripts\\utilities\\invoke-cloud-agent\.ps1\s+-Provider\s+(\w+)\s+-Agent\s+"([^"]+)"\s*```/g,
    replacement: '```bash\nnpx tsx src/invoke-cloud-agent.ts --provider $1 --agent "$2"\n```'
  },
  { 
    pattern: /```powershell\s*\.\\scripts\\utilities\\invoke-cloud-agent\.ps1\s+-Provider\s+(\w+)\s+-StrictJson\s+-Command\s+"([^"]+)"\s*```/g,
