...
const AGENT_CONFIG: DomainAgentConfig = {
  ...
  execute: (task, context, prompt) => {
    ...
    const intent =
      normalized.includes('job') || normalized.includes('description') || normalized.includes('jd') ? 'JOB_DESCRIPTION' :
      normalized.includes('interview') ? 'INTERVIEW' :
      normalized.includes('onboard') ? 'ONBOARDING' :
      normalized.includes('remote') || normalized.includes('policy') ? 'POLICY' :
    ...
    const flags: DomainOutput['flags'] = [
      { severity: 'info', message: 'HR processes must be documented: process owner, steps with timeline, decision criteria, escalation path, audit trail, retention period' }
    ];
    if (normalized.includes('job') || normalized.includes('descrip')) {
      flags.push({ severity: 'warn', message: 'Verify inclusive language table before publishing (HR.md §Inclusive Language)' });
    }

    const artifacts = [
      {
        name: 'hr-workflow',
        content: `# HR Workflow — ${intent.replace(/_/g, ' ')}\n\n` +
          `**Task:** ${task}\n\n` +
          `## Critical Rules\n\n` +
    ...
    return {
      ...
      checklist,
      artifacts,
      evidence: ['HR.md domain prompt loaded', 'Inclusive language + compliance checklists applied'],
      flags
    };
  }
};
