...
if ($evHist -and $evHist.events) {
    foreach ($e in $evHist.events) {
        $ts = $null
        if ($e.timestamp) { try { $ts = [datetime]::Parse($e.timestamp) } catch { Write-Debug "Exception caught: <#
.SYNOPSIS
    Export persisted metrics to a dedicated analytical store.
...
if (Test-Path $tokenCsv) {
    try {
        ...
        foreach ($line in $csvLines) {
            ...
            if ($parts.Count -lt 2) { continue }
            $ts = $null
            try { $ts = [datetime]::Parse($parts[0].Trim()) } catch { Write-Debug "Exception caught: <#
.SYNOPSIS
    Export persisted metrics to a dedicated analytical store.
