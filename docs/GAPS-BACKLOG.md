...
> comportamiento errático, o necesita revisión. Cada entrada incluye criticidad y etiquetas
> para priorizar según amerite.
>
...
> tabla `backlog_items`). Este archivo .md es un snapshot de documentación. Para consultas en
> tiempo real, usar `npm run backlog:list`, `npm run backlog:search`, o `npm run backlog:stats`.
> Ver `apps/web-dashboard/server/database/repositories/BacklogRepo.ts` para la implementación.
>
...
**Severidad:** 🔴 HIGH
**Etiquetas:** `doc-sync` `presentations` `ps1-migration`
...
**Archivos afectados:**
- `docs/presentations/dashboard.html` — referencias a `dashboard-start.ps1`, `dashboard-ws-autostart.ps1`, `dashboard-stop.ps1`
...
**Acción:** Reemplazar todas las referencias `.ps1` por sus equivalentes en `npx tsx` o comandos npm.
...
**Severidad:** 🔴 CRITICAL
**Etiquetas:** `agent-config` `claude-md` `broken-reference`
...
**Impacto:** Si un agente nuevo sigue esta CLAUDE.md al pie de la letra, falla en el primer
comando y no puede operar.
...
**Severidad:** 🔴 HIGH
**Etiquetas:** `policy` `ps1-migration` `outdated`
...
**Severidad:** 🟡 MEDIUM
**Etiquetas:** `performance` `pipeline` `startup`

**Descripción:** `session-autostart.ts` ejecuta múltiples pasos lazy en background. En ocasiones
el pipeline tarda >30s en completarse, especialmente cuando CodeGraph o Engram tienen que
reindexar. No hay feedback visual al usuario durante la espera.
...
**Severidad:** 🟢 LOW
**Etiquetas:** `dashboard` `usability` `config`

**Descripción:** El dashboard WS server necesita `WS_PORT` env var o usa puerto fijo. En entornos
con múltiples instancias puede haber conflicto. `dashboard-common.ps1` tiene `Get-FreePort()`
pero está en PS1 (no migrado a TS).
...
**Severidad:** 🟡 MEDIUM
**Etiquetas:** `presentations` `missing-page` `book`

**Descripción:** El book de presentaciones tiene 10 páginas en el navbar pero la página 10
(Stack Health & Metrics / health.html) no ha sido creada. En index.html está marcada como
"próximamente".
...
**Severidad:** 🟡 MEDIUM
**Etiquetas:** `git-hooks` `stale-file` `cleanup`

**Descripción:** El archivo `.git/hooks/pre-commit-prompt-optimize.ps1` es un hook antiguo que
referencia `scripts\utilities\semantic-compression.ps1` (que ya no existe). No es ejecutado
por lefthook (los hooks reales están gestionados por lefthook), pero está presente y puede
causar confusión.
...
**Severidad:** 🟢 LOW
**Etiquetas:** `testing` `performance` `ci`
