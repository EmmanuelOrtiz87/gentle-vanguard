...
**Why**: Each tool has different capabilities (skill tool, mem tools, file access). Detection ensures correct routing, compression, and security profiles. OS detection prevents wasted tokens from running wrong-platform commands.
...
6. **Read `scripts/.session/startup-summary.json`** (generated by autostart Phase 9) — parsed post-autostart data: `isPeakHour`, `sessionId`, `workspaceClean`, `engramRunning`, `platform`, `shell`, `pathSeparator`
