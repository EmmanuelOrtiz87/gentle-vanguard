...
When the owner doesn't have QuickBooks or PayPal connected, they can upload exported CSV files. This
document specifies what columns to expect and how to handle variations.
...
- PayPal: Activity → Download (CSV) — use "Transaction ID, Date, Name, Type, Status, Currency,
  Amount"
- Square: Reports → Transactions → Export
- Shopify: Orders → Export

If the export has more columns, ignore the extras. If a required column is missing, ask the owner
