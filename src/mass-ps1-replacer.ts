  ...
  for (const ext of EXTENSIONS) {
    try {
      const output = runSyncShell(
        `find . -name "*${ext}" -type f 2>/dev/null | head -500`,
        { cwd: ROOT }
      ).stdout;
      ...
      try {
        const output = runSync(
          'powershell',
          ['-NoProfile', '-Command', `Get-ChildItem -Recurse -Filter "*${ext}" | Select-Object -ExpandProperty FullName`],
          { cwd: ROOT }
        ).stdout;
        files.push(...output.trim().split('\n').filter(f => f));
      } catch {}
...
function processFile(filePath: string, dryRun: boolean): { changes: number; action: string } {
  ...
  content = content.replace(/pwsh\s+-NoProfile\s+-ExecutionPolicy\s+Bypass\s+-File\s+[^\s]+\.ps1/g, 'npx tsx src/gv.ts');
...
function main() {
  ...
  for (const file of files.slice(0, 100)) { // Limitar a 100 archivos por seguridad
