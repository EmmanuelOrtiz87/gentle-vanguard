...
const EXAMPLE = {
  ...
  context:
    'TypeScript + React 18 monorepo. vitest tests. Stripe payments. Public API contracts must not break.',
...
export default function App() {
  ...
  const prompt = useMemo(() => {
    ...
    L.push('# Role');
    L.push(
      `Act as ${role.trim() || '[FILL: who the assistant should be]'}. Your goal: ${goal.trim() || '[FILL: the concrete task in one sentence]'}.`,
    );
    L.push('', '# Task');
    L.push(
  ...
  const openInEditor = (p: PromptRow) => {
    setEditingId(p.id);
    setType(p.type || 'review');
    setRole(p.role ?? '');
    setGoal(p.goal ?? '');
    setContext(p.context ?? '');
    setCriteria(p.criteria ?? '');
    setFormat(p.format || OUTPUT_FORMATS[0]);
    setTone(p.tone ?? '');
  ...
  const tabBtn = (t: Tab, label: string, Icon: typeof Sparkles) => (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
        tab === t
          ? 'bg-cyan-400/15 text-cyan-200 border border-cyan-400/50'
          ...
          <div className="gv-brand">
            ...
            <span>
              ...
              <small className="block text-slate-400 text-xs">
                {t('appName')} · {t('brandSub')}
              </small>
            </span>
          </div>
          <nav className="gv-view-tabs">
            {tabBtn('create', t('create'), Sparkles)}
          ...
          <div className="gv-panel p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                ...
                >
                  <Wand2 className="w-4 h-4" /> {t('example')}
                </button>
                <button
                  type="button"
                  onClick={newPrompt}
                ...
                >
                  <Pencil className="w-4 h-4" /> {t('newPrompt')}
                </button>
              </div>
            </div>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <section className="space-y-3">
                ...
                {(
                  [
                    ['ps-role', t('assistantRole'), role, setRole, 'Quién debe ser el asistente'],
                    ['ps-goal', t('goal'), goal, setGoal, 'Una frase concreta'],
                    ['ps-tone', t('tone'), tone, setTone, 'ej. directo, técnico, sin relleno'],
                  ] as const
                ).map(([id, label, val, set, ph]) => (
                  <div key={id}>
                    <label className="block text-xs font-semibold text-slate-300 mb-1" htmlFor={id}>
                      {label}
                    </label>
                    <input
                      id={id}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Repo, stack, restricciones…"
                  />
                </div>
                <div>
                  <label
              ...
              <section className="flex flex-col">
                ...
                {status && (
                  <p className="mt-2 text-xs text-cyan-300" role="status">
                    {status}
                  </p>
                )}
              </section>
        ...
        {tab === 'library' && (
          <div className="gv-panel p-5">
            ...
            >
              ...
              <button
                type="submit"
                className="rounded-full border border-cyan-400/50 px-4 py-2 text-sm font-semibold text-cyan-200 hover:bg-cyan-400/10"
              >
                {t('search')}
              </button>
            </form>
              ...
              {prompts.map((p) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <button
                        ...
                        className={
                          p.favorite ? 'text-amber-300' : 'text-slate-500 hover:text-amber-200'
                        }
                      >
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    ...
                    {p.tags
                      ...
                      .map((t) => (
                        <span
                          key={t}
                          className="text-[10px] px-2 py-0.5 rounded-full bg-cyan-400/10 text-cyan-300 border border-cyan-400/20"
                        >
                          {t.trim()}
                        </span>
                      ))}
                  ...
                  <p className="text-[10px] text-slate-500 mt-2">
                    Actualizado: {p.updated_at?.slice(0, 16).replace('T', ' ')}
                  </p>
                </article>
              ))}
        ...
        {tab === 'guides' && (
          <div className="gv-panel p-6 space-y-8">
            <div>
              ...
              <ol className="list-decimal list-inside space-y-2 text-sm text-slate-300">
                <li>
                  ...
                  de IA (ChatGPT, Claude, Gemini, Copilot, ZCode).
                </li>
                <li>
                  <strong>Delegar a un subagente</strong>: en el stack, usá{' '}
                  <code className="text-cyan-300">
                    npm run delegate:run -- --task "&lt;objetivo del prompt&gt;"
                  </code>{' '}
                  y el router elige el agente por dominio.
                </li>
                <li>
                  <strong>Guardar como instrucción persistente</strong>: convertí el prompt en la
                  instrucción de sistema de tu herramienta (AGENTS.md, GEMINI.md, custom
                  instructions).
                </li>
              </ol>
            </div>
            <div>
              ...
              <ol className="list-decimal list-inside space-y-2 text-sm text-slate-300">
                <li>
                  Entrá a Gemini → <strong>Gemas</strong> → <strong>Nueva Gema</strong> (o Gem
                  manager).
                </li>
                <li>
                  <strong>Nombre</strong>: el objetivo en 3-5 palabras (ej. «Revisor de código
                  senior»).
                </li>
                <li>
                  <strong>Instrucciones</strong>: pegá el prompt completo generado acá — la
                  estructura Rol / Tarea / Contexto / Criterios / Formato / Verificación es ideal
                  porque deja claro qué hacer y cómo autoevaluarse.
                </li>
                <li>
                  <strong>Conocimiento</strong> (opcional pero potente): subí archivos de contexto —
                  specs, convenciones del repo, ejemplos de buen resultado. La Gema los usa como
                  referencia permanente.
                </li>
                <li>
                  <strong>Probar y refinar</strong>: dale una tarea real; si falla un criterio,
                  ajustá esa sección del prompt (editándolo acá en el Studio) y actualizá las
                  instrucciones de la Gema.
                </li>
                <li>
                  <strong>Reutilizar</strong>: cada Gema queda disponible como chat propio — es tu
                  «agente» para esa tarea, sin re-escribir el prompt nunca más.
                </li>
              </ol>
            </div>
            <div>
              ...
              <ul className="list-disc list-inside space-y-2 text-sm text-slate-300">
                ...
                <li>
                  <strong>Formato estricto</strong> cuando la salida alimenta otra herramienta (JSON
                  con schema).
                </li>
                <li>
                  <strong>Verificación obligatoria</strong>: pedir auto-revisión contra criterios
                  reduce drásticamente respuestas mediocres.
                </li>
                <li>
                  <strong>Contexto mínimo suficiente</strong>: más contexto no es mejor; el contexto{' '}
                  <em>correcto</em> sí.
                </li>
              </ul>
            </div>
