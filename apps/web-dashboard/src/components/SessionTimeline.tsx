...
import { Clock, ChevronDown, ChevronRight, Circle, Zap, GitBranch, Users, Activity, Shield } from 'lucide-react';
  ...
  if (events.length === 0) {
    return (
      <div className="text-center py-8 text-gray-400 dark:text-gray-500">
        ...
        <p className="text-xs mt-1">Events appear when agent activities, sessions, or workflows are triggered</p>
      </div>
    );
  }
        ...
        return (
          <div key={`${evt.timestamp}-${i}`} className="flex gap-4">
            ...
            <div className={`flex-1 pb-6 ${isLast ? 'pb-0' : ''}`}>
              ...
              {evt.execution_id && (
                <p className="text-[10px] text-gray-400 font-mono mt-0.5">exec: {evt.execution_id}</p>
              )}
