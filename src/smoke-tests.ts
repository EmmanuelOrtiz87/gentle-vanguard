...
function log(msg: string) { console.log(CYAN + `[SMOKE] ${msg}` + RESET); }
...
const smokeTests: SmokeTest[] = [
  ...
  
  {
    ...
    run: () => {
      ...
      if (!existsSync(join(ROOT, scriptPath))) {
        return { passed: false, details: `Script not found: ${scriptPath}`, duration: Date.now() - startTime };
      }
      
      const r = spawnWithPlatform('npx', ['tsx', scriptPath], 10000);
      
      const passed = r.status === 0 && (r.stdout?.includes('HEALTHY') || r.stdout?.includes('healthy'));
      
      return {
        passed,
        details: passed ? 'DB health check executed successfully' : `Failed: ${r.stderr || r.error || 'timeout'}`,
        duration: Date.now() - startTime
      };
    }
  },
  
  {
    ...
    run: () => {
      ...
      if (!existsSync(ckptDir)) {
        return { passed: true, details: 'No checkpoints yet (expected on fresh session)', duration: 0 };
      }
  ...
  
  {
    ...
    description: 'Verifica que spawnSync(npx) funciona en Windows (con timeout extendido por posible instalación)',
    critical: true,
    timeout: 20000,
    run: () => {
      const startTime = Date.now();
      ...
      if (!passed) {
        console.log(`  Debug: status=${r.status}, error=${r.error?.message}, stderr=${r.stderr?.substring(0, 200)}`);
      }
      
      return {
        passed,
        details: passed ? 'npx spawn works' : `Failed: ${r.stderr || r.error?.message || 'timeout'}`,
        duration: Date.now() - startTime
      };
    }
  },
  
  {
    ...
    run: () => {
      ...
      return {
        passed,
        details: passed ? 'Dashboard WS responding' : 'Dashboard WS not running (may be OK if not started)',
        duration: Date.now() - startTime
      };
    }
  },
...
function runSmokeTests(): SmokeReport {
  ...
  const criticalMsg = report.critical > 0 
    ? `${RED} ${report.critical} CRITICAL FAILED${RESET}` 
    : `${GREEN} 0 critical issues${RESET}`;
  
  log(`${report.passed}/${report.total} passed | ${report.failed} failed | ${report.critical} critical`);
  log(`Duration: ${report.duration}ms`);
  log(`Status: ${report.failed === 0 ? GREEN + 'ALL PASS' + RESET : RED + 'SOME FAILURES' + RESET}`);
