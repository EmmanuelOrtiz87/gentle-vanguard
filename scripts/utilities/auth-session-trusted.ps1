...
<#
.SYNOPSIS
    Session Authentication with Trusted User Support - Validates owner before restricted operations
.DESCRIPTION
    Flow:
    1. Check if user is in trusted users list
    2. If trusted and operation is exempt -> allow without auth
    3. If not trusted or operation not exempt -> follow normal auth flow
    4. Check if already authenticated in this session
    5. If not, check lockout status (rate limiting)
    6. Require API key OR security questions
    7. If API key correct -> authenticate session
    8. If API key wrong -> log failed attempt, offer security questions
    9. Security questions -> 3/3 correct required, then reveal API key
    10. If anything wrong -> DENIED
    11. Integrity check on owner-auth.json (SHA256 hash validation)
    12. Supports DPAPI encryption via secure-auth.ps1
#>
...
$trustedUserCheckScript = Join-Path $repoRoot "scripts\utilities\trusted-user-check.ps1"
...
function Test-SecurityQuestions {
    ...
    $hash = [System.Security.Cryptography.SHA256]::Create()
    if (-not (Test-Path $authPath)) { return 0 }
    $auth = Get-Content $authPath -Raw | ConvertFrom-Json
    $correct = 0
...
function Test-TrustedUser {
    param([string]$Operation)
    
    if (-not (Test-Path $trustedUserCheckScript)) {
        return $false
    }
    
    try {
        $result = & $trustedUserCheckScript -Operation $Operation | ConvertFrom-Json
        return $result.isTrusted -and $result.isOperationExempt
    } catch {
        return $false
    }
}
...
# TRUSTED USER CHECK
# ======================================================
if (Test-TrustedUser -Operation $OperationContext) {
    Write-Host "[OK] Trusted user bypass - Operation allowed without authentication" -ForegroundColor Green
    exit 0
}
...
if (-not $ApiKey -and -not $UseSecurityQuestions) {
    Show-AuthRequired -Context $OperationContext
...
if ($UseSecurityQuestions) {
    ...
    if (-not (Test-Path $authPath)) {
        Show-AuthError -Reason "Authentication file not found"
        exit 1
    }
