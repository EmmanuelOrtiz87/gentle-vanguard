...
let invocationType: 'RequestResponse' | 'Event' | 'DryRun' = 'RequestResponse';
...
function writeLog(message: string, level: 'INFO' | 'WARN' | 'ERROR' | 'SUCCESS' = 'INFO') {
...
export function stopTracingSpan(name: string, success: boolean, error?: string) {
  const tracer = join(ROOT, 'src/tracing-instrument.ts');
...
export function logAudit(status: string, detail: string) {
  const audit = join(ROOT, 'src/audit-pipeline.ts');
  if (existsSync(audit)) {
    spawnSync('npx', ['tsx', audit, 'log', '-EventType', 'skill.exec', '-Component', 'cloud', '-Operation', 'azure-invoke', '-Actor', 'system', '-Target', skillId, '-Status', status, '-Message', detail, '-Quiet'], { cwd: ROOT, stdio: 'pipe', timeout: 15000 });
  }
}
...
function httpsPost(url: string, body: string, headers: Record<string, string>): Promise<{ statusCode: number; data: string }> {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(opts, (res) => {
      ...
      res.on('end', () => resolve({ statusCode: res.statusCode ?? 500, data: chunks.join('') }));
    });
...
async function invokeSkillOnAzureFunction(skill: string, input: unknown): Promise<InvokeResult> {
...
async function invokeWithRetry(fn: () => Promise<InvokeResult>, retries: number): Promise<InvokeResult> {
...
function recordCloudMetrics(provider: string, duration: number, success: boolean, cost: number) {
  let metrics: { executions: MetricsEntry[] } = { executions: [] };
...
function saveSessionStateToCosmos(sessionState: unknown) {
  writeLog('Saving session state simulation to Cosmos backup', 'INFO');
  try {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    switch (key.toLowerCase().replace(/-/g, '')) {
      ...
      case 'skill_input':
        if (hasValue) {
          const raw = argv[++i];
          try { skillInput = JSON.parse(raw); } catch { skillInput = raw; }
        }
        break;
      case 'invocationtype':
      case 'invocation_type':
        if (hasValue) { invocationType = argv[++i] as typeof invocationType; }
        break;
      case 'functionurl':
      case 'function_url':
