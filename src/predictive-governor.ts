...
const DEFAULT_CONFIG = {
  prediction: { windowDays: 14, minDataPoints: 5, seasonalityDetection: true, trendDetection: true, anomalyThreshold: 2.5 },
  prewarming: { enabled: true, codegraphPrefetchThreshold: 0.7, knowledgeBasePrefetch: false, leadMinutes: 15 },
  budgetAdjustment: { enabled: true, maxIncreasePct: 30, maxDecreasePct: 20, peakMultiplier: 1.3, offPeakMultiplier: 0.8, floorTokens: 5000, ceilingTokens: 200000 },
  outputDir: GOV_DIR,
};
...
function collectUsageHistory(log: LogFn): UsageRecord[] {
  ...
  if (existsSync(AUDIT_DIR)) {
    const files = readdirSync(AUDIT_DIR).filter(f => f.endsWith('.jsonl')).sort().slice(-20);
...
function analyzeUsagePatterns(
  ...
  patterns.push({
    ...
    data: dailyTokens,
    recommendation: peakDays.length > 0
  ...
  if (records.length >= 5 && config.prediction.trendDetection) {
    ...
    const firstAvg = firstHalf.reduce((s, r) => s + r.tokensUsed, 0) / Math.max(firstHalf.length, 1);
    const secondAvg = secondHalf.reduce((s, r) => s + r.tokensUsed, 0) / Math.max(secondHalf.length, 1);
  ...
  for (const d of peakDays.length > 0 ? peakDays : [0, 1, 2, 3, 4, 5, 6]) {
    ...
    const highHours = dayHourlyTokens.map((t, h) => ({ hour: h, tokens: t, ratio: t / maxDayTokens }))
...
function main(): void {
  ...
  log(`  Current budget: ${currentBudget.limit}, used: ${currentBudget.used}, remaining: ${currentBudget.remaining}`);
  ...
  if (args.mode === 'all' || args.mode === 'analyze') {
    ...
    for (const pw of peakWindows.slice(0, 3)) {
      const dayNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
      log(`    ${dayNames[pw.dayOfWeek]} ${pw.hourStart}:00-${pw.hourEnd}:00 (intensity: ${(pw.intensity * 100).toFixed(0)}%)`);
    }
  }
  ...
  if (args.mode === 'all' || args.mode === 'adjust') {
    ...
    if (budgetAdjustment) {
      log(`  Budget: ${budgetAdjustment.currentBudget} → ${budgetAdjustment.suggestedBudget} (${budgetAdjustment.reason})`);
    } else {
  ...
  const output: GovOutput = {
    ...
    summary: {
      ...
      avgTokensPerSession: records.length > 0
        ? Math.round(records.reduce((s, r) => s + r.tokensUsed, 0) / records.length)
        : 0,
      peakHours: peakWindows.length > 0
        ? peakWindows.slice(0, 3).map(pw => `${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][pw.dayOfWeek]} ${pw.hourStart}:00`).join(', ')
        : 'Not yet determined',
  ...
  if (!args.quiet) {
    console.log(JSON.stringify({
      ...
      budgetAdjustment: output.budgetAdjustment ? `${output.budgetAdjustment.currentBudget}→${output.budgetAdjustment.suggestedBudget}` : 'none',
