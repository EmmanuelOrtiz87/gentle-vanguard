...
export class OperationalMetricsTracker {
  ...
  static calculateMetrics(fromDate?: string, toDate?: string): DailyMetrics {
    ...
    const linesAdded = gitEvents.reduce((sum, e) => sum + ((e.metadata?.linesAdded as number) || 0), 0);
    const linesDeleted = gitEvents.reduce((sum, e) => sum + ((e.metadata?.linesDeleted as number) || 0), 0);
    const avgTimeBetweenCommits = calculateAvgTimeBetween(gitEvents);
    
    // Efficiency
    const toolEvents = events.filter(e => e.type === 'tool');
    const avgToolLatency = toolEvents.reduce((sum, e) => sum + e.durationMs, 0) / toolEvents.length || 0;
    ...
    const buildSuccessRate = (buildEvents.filter(e => e.success).length / buildEvents.length * 100) || 100;
    const testEvents = events.filter(e => e.type === 'test');
    const testPassRate = (testEvents.filter(e => e.success).length / testEvents.length * 100) || 100;
...
function getFastestTool(events: OperationEvent[]): string {
  ...
  const fastest = validEvents.reduce((min, e) => e.durationMs < min.durationMs ? e : min, validEvents[0]);
...
function getSlowestTool(events: OperationEvent[]): string {
  ...
  const validEvents = events.filter(e => e.durationMs && e.durationMs > 0);
  if (validEvents.length === 0) return 'N/A';
  const slowest = validEvents.reduce((max, e) => e.durationMs > max.durationMs ? e : max, validEvents[0]);
