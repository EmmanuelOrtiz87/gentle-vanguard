...
**Agnostic Principle**: Any memory system can be plugged in (Engram, files, databases, vector
stores).
...
**Agnostic Principle**: Skills are declarative markdown with triggers - any agent can interpret
them.
...
**Purpose**: Standardized protocol for tools, resources, and prompts between AI models and external
systems.

**Agnostic Principle**: Any MCP-compliant server can be integrated regardless of language or
platform.
...
**Agnostic Principle**: Tools are defined by interface, not implementation. Any tool with matching
signature works.
...
**Agnostic Principle**: Any AI agent (Claude, GPT, local models) can use this topology if it
supports the tool interface.
