...
function getFileAgeHoursCached(filePath: string): number {
  const age = getCached(`age:${filePath}`, () => {
    try {
  ...
  }, 5000); // Shorter TTL for freshness
...
async function checkDashboardWs() {
  ...
  addResult('dashboard-ws', 'WS server process', hasPid ? 'PASS' : 'WARN', hasPid ? 'PID file exists' : 'No PID file', hasPid ? 'ok' : 'start');
...
async function checkMlEmbeddings() {
  ...
  if (ageH === -1) {
    ...
    addResult('ml-embeddings', 'skill-embeddings.json freshness', 'WARN', `${ageH.toFixed(1)} hours`, 'rebuild');
  } else {
  ...
  if (fileExistsCached(mlDir)) {
    const files = getCached(`files:${mlDir}`, () => {
      try {
    ...
    addResult('ml-embeddings', 'embedding files', files > 0 ? 'PASS' : 'WARN', `${files} files`, 'rebuild');
  }
  
  // Script existence (cached)
  ['src/skills/skill-embedder.ts', 'src/ml-router.ts'].forEach(s => {
    addResult('ml-embeddings', basename(s), fileExistsCached(join(ROOT, s)) ? 'PASS' : 'FAIL', '', 'manual');
  });
  
  // Parse once (cached)
  const idx = readJsonCached(mlIndex);
  if (idx) {
    addResult('ml-embeddings', 'index parseable', 'PASS', `${Object.keys(idx).length} skills`, 'ok');
  }
}
...
async function checkEngram() {
  ...
  addResult('engram', 'reindex script', fileExistsCached(ragReindexTs) ? 'PASS' : 'FAIL', '', 'manual');
  ...
  if (fileExistsCached(ragLog)) {
    ...
    addResult('engram', 'reindex freshness', status, `${logAge.toFixed(1)} hours`, status === 'PASS' ? 'ok' : 'reindex');
  } else {
  ...
  addResult('engram', 'engram directory', fileExistsCached(join(userProfile, '.engram')) ? 'PASS' : 'FAIL', '', 'manual');
...
async function checkMcp() {
  ...
  checks.forEach(([comp, name, p]) => {
    addResult(comp as string, name, fileExistsCached(join(ROOT, p)) ? 'PASS' : 'FAIL', '', 'manual');
  });
  ...
  addResult('mcp', 'config files', found === mcpConfigs.length ? 'PASS' : 'WARN', `${found}/${mcpConfigs.length}`, 'verify');
...
async function checkSessionPipeline() {
  ...
  [
    ...
    addResult('session', basename(s), fileExistsCached(join(ROOT, s)) ? 'PASS' : 'FAIL', '', 'manual');
  });
  
  addResult('session', 'autostart config', fileExistsCached('config/session-autostart.config.json') ? 'PASS' : 'FAIL', '', 'manual');
}
...
async function checkConfigs() {
  ...
  addResult('configs', 'core configs', passed === configs.length ? 'PASS' : 'WARN', `${passed}/${configs.length}`, 'fix');
}
...
async function checkTracing() {
  ...
  addResult('tracing', 'instrumentation script', fileExistsCached(join(ROOT, 'src/tracing-instrument.ts')) ? 'PASS' : 'FAIL', '', 'verify');
  
  // Skip file count in fast mode
  addResult('tracing', 'trace files', fileExistsCached(join(telemetryDir, 'traces')) ? 'PASS' : 'WARN', 'Fast mode', 'ok');
}
...
async function checkStatePersistence() {
  ...
  addResult('state-persistence', 'scripts', 
    fileExistsCached('src/checkpoint-manager.ts') && fileExistsCached('src/snapshot-manager.ts') ? 'PASS' : 'FAIL', 
    'Fast mode', 'verify'
  );
}
...
async function checkGentleVanguardDb() {
  ...
  if (dbExists) {
    const size = getCached(`dbsize:${dbPath}`, () => {
      try { return (statSync(dbPath).size / 1024 / 1024).toFixed(2); } catch { return '0'; }
    });
...
async function checkAuditPipeline() {
  ...
  addResult('audit', 'pipeline script', fileExistsCached(join(ROOT, 'src/infrastructure/audit-pipeline.ts')) ? 'PASS' : 'FAIL', '', 'verify');
...
function generateReport(outputPath?: string) {
  ...
  results.forEach(r => {
    ...
    if (r.status === 'FAIL') { c.status = 'ISSUES'; c.fails++; }
...
async function runAllChecks() {
  ...
  await Promise.all(checks.map(async (check) => {
    try { await check(); } 
    catch (e) { addResult('system', check.name, 'FAIL', String(e), 'manual'); }
  }));
