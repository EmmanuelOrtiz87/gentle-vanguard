---
name: run-campaign-skill
description: >
  Knowledge work plugin from small-business department.
metadata:
  source: knowledge-work-plugins
  original-name: run-campaign
  department: small-business
---

Run the full campaign pipeline by chaining three skills in order. The owner approves at each handoff
— never roll past a gate without explicit confirmation.
...
3. Generate on-brand Canva assets for each post (apply each on screen for owner approval before
   moving on).
4. Draft caption copy for each post.
5. Stage the scheduled send in HubSpot (do NOT send — staging only).
6. Present the staged campaign to the owner. Wait for explicit "approved, send to segment X" before
   Step 3.
...
- If any connector is unreachable (QuickBooks, PayPal, Canva, HubSpot), stop, report which connector
  failed, and ask whether to retry or abort.

## Output

End the run with a one-paragraph recap: revenue dip identified, posts generated, segment size, calls
booked. Link to the HubSpot campaign URL once sent.
