  ...
  tools: [
    {
      name: 'list_tokens',
      description:
        'List Gentle-Vanguard design tokens (colors, typography, spacing, radius, elevation, motion, zIndex). Filter by category and theme.',
      inputSchema: {
        type: 'object',
        properties: {
          category: {
    ...
    },
    {
      name: 'get_component',
      description:
        'Get the API (props + variants) of a Gentle-Vanguard React component. Returns TypeScript interface and usage examples.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
    ...
    },
    {
      name: 'audit_design',
      description:
        'Run impeccable design audit on a path or URL. Detects 59 anti-patterns (AI slop tells, typography issues, color/contrast, layout, motion, quality). Returns JSON or human-readable findings.',
      inputSchema: {
        type: 'object',
        properties: {
          target: {
    ...
    },
    {
      name: 'sync_design',
      description:
        'Regenerate design tokens in all consuming apps (gv-analytics, academy-web, etc). Runs `npm run gv:design` under the hood.',
      inputSchema: {
        type: 'object',
        properties: {
          app: {
    ...
    },
    {
      name: 'get_design_md',
      description:
        'Return the canonical Gentle-Vanguard DESIGN.md (the design language reference for the entire ecosystem).',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    },
    {
      ...
      description:
        'List brand asset waivers — patterns that impeccable would flag as anti-patterns but are intentional Gentle-Vanguard brand signatures (wordmark gradient, grid background, etc).',
      inputSchema: {
        type: 'object',
        properties: {},
      },
