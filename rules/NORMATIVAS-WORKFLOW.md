...
- `.session/.active-session.json` is a historical/compatibility persistence marker, not the
  lifecycle authority.
- Token aggregates and transactions are authoritative in Nexus (`.runtime/gentle-vanguard.db`); tool
  JSONL rollouts are the raw usage authority for JSONL-producing tools, while generated JSON
  snapshots are derived reports.
