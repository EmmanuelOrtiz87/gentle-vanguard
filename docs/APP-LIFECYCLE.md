...
Los scripts detectan el project root (`../../` desde la app), escriben/leen pidfiles en `.runtime/`
(mismos nombres que usa command-center) y nunca abren ventanas cmd. Logs en `.runtime/app-*.log`.
...
- **Delegación**: design-hub (scripts node propios), web-dashboard (`src/ops/dashboard-start.ts` /
  ...
  `src/cli/gv.ts cc stop`) reutilizan los mecanismos existentes del monorepo en vez de duplicarlos.
