...
Si `required: true` y el paso falla → el pipeline se detiene (`exit 1`). Si `required: false` y el
paso falla → se registra warning y continúa.
...
Los objetos de `ConvertFrom-Json` son PSCustomObject. Asignar propiedades nuevas con
`$obj.newProp = value` puede fallar si la propiedad no existe.
