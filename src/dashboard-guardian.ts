...
function isDashboardRunning(): boolean {
  try {
    // Check HTTP endpoint
    const result = runSyncShell(`curl -s -o nul -w "%{http_code}" http://localhost:${PORT}/health`, {
      timeout: 3000,
...
async function main() {
  ...
  while (consecutiveFailures < MAX_RETRIES) {
    ...
    if (!healthy) {
      console.log(`[GUARDIAN] ${new Date().toISOString()}: Unhealthy (PID: WS=${pids.ws}, WD=${pids.watchdog})`);
