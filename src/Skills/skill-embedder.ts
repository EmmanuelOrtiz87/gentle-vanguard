...
function parseArgs(): SkillEmbedderArgs {
  ...
  return {
    ...
    outputPath: extractArg(raw, '--output-path') || join(projectRoot, '.atl', 'skill-embeddings.json'),
  };
}
...
function parseSkillRegistry(path: string): Record<string, SkillEntry> {
  ...
  for (const line of lines) {
    ...
    if (/^## Skill-Agent Mapping/.test(line)) { inMappingTable = true; continue; }
    ...
    let triggers: string[] = [];
    if (triggerPart) {
      triggers = triggerPart.split(',').map(t =>
...
function getAgentKeywords(path: string): Record<string, string[]> {
  ...
  for (const agentCode of Object.keys(km)) {
    ...
    for (const val of km[agentCode]) {
      ...
      if (clean && !/^(when|for|or|and|after|in|user mentions|if the|when creating|when writing|when working|when using|when building|when a|when you|when orchestrator|when managing|when planning|when adding)/.test(clean)) {
        items.push(clean);
      }
    }
...
function addSkillsFromConfig(skills: Record<string, SkillEntry>, delegationConfigPath: string): Record<string, SkillEntry> {
...
function buildSkillText(skills: Record<string, SkillEntry>, agentKeywords: Record<string, string[]>): Record<string, SkillTextInfo> {
  ...
  for (const [skillName, skill] of Object.entries(skills)) {
    ...
    for (const t of skill.triggers) {
      if (t) { baseParts.push(t); fullParts.push(t); }
    }
...
function buildSkillVectors(skillTexts: Record<string, SkillTextInfo>, vocabulary: Record<string, number>, idf: Record<string, number>): VectorResult {
...
function saveEmbeddings(
  ...
  for (const [skillName, v] of Object.entries(vectors)) {
    ...
    for (const [k, val] of Object.entries(v.vector)) vecObj[k] = Math.round(val * 1000000) / 1000000;
  ...
  console.log(`Embeddings saved to ${outputPath} (${skillsOut.length} skills, ${vocabList.length} vocabulary terms)`);
}
...
function main(): void {
  ...
  saveEmbeddings(vectorResult.vectors, vectorResult.charNgrams, vocabResult.vocabulary, vocabResult.idf, args.outputPath);
