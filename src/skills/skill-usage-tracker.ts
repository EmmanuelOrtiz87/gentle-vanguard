...
interface SkillMetric {
  ...
  failureCount: number;
  failurePatterns: Array<{ timestamp: string; errorType: string; description: string; fixApplied: string | null }>;
...
function getInitialMetric(name: string): SkillMetric {
  return { skillName: name, useCount: 0, lastUsedAt: null, failureCount: 0, failurePatterns: [], avgTokensUsed: 0, successRate: 1.0, lastOutcome: null };
}
...
function readMetric(name: string): SkillMetric | null {
  ...
  if (existsSync(path)) {
    try { return JSON.parse(readFileSync(path, 'utf-8')); } catch { console.warn(`Corrupt metric for ${name}, reinitializing`); }
  }
  return null;
}
...
function recordFailure(name: string, errorType: string, description: string): void {
  ...
  metric.successRate = Math.round((1.0 - (metric.failureCount / Math.max(metric.useCount, 1))) * 100) / 100;
  if (errorType && description) metric.failurePatterns.push({ timestamp: new Date().toISOString(), errorType, description, fixApplied: null });
  saveMetric(name, metric);
  console.log(`[WARN] ${name} failed (failures: ${metric.failureCount})`);
  if (metric.failureCount % 3 === 0) console.log(`[NUDGE] ${name} has ${metric.failureCount} failures — consider checking skill`);
...
function record(name: string, outcome: string | null, tokenCount: number): void {
  ...
  if (tokenCount > 0) {
    metric.useCount++;
    metric.avgTokensUsed = Math.round(((metric.avgTokensUsed * (metric.useCount - 1)) + tokenCount) / metric.useCount * 10) / 10;
  }
  saveMetric(name, metric);
...
function checkNudgeConditions(): void {
  ...
  for (const f of files) {
    try {
      ...
      if (m.failureCount >= 3) console.log(`[NUDGE] ${m.skillName}: ${m.failureCount} failures, rate ${m.successRate}`);
      if (m.useCount >= 10 && m.successRate < 0.7) console.log(`[NUDGE] ${m.skillName}: Declining success rate (${m.successRate}) after ${m.useCount} uses`);
    } catch { /* */ }
  }
}
...
function showReport(): void {
  ensureDir(usageDir);
  const files = readdirSync(usageDir).filter(f => f.endsWith('.json')).sort();
  if (files.length === 0) { console.log('[INFO] No usage data found. Run scan first.'); return; }
  console.log(`\n=== Skill Usage Report ===`);
  console.log(`${'Skill'.padEnd(30)} ${'Uses'.padStart(8)} ${'Failures'.padStart(9)} ${'Rate'.padStart(10)} ${'Last'.padStart(8)}`);
  console.log(`${'----'.padEnd(30)} ${'----'.padStart(8)} ${'--------'.padStart(9)} ${'----'.padStart(10)} ${'----'.padStart(8)}`);
  for (const f of files) {
    try {
      const m: SkillMetric = JSON.parse(readFileSync(join(usageDir, f), 'utf-8'));
      const last = m.lastUsedAt ? m.lastUsedAt.slice(0, 10) : 'never';
      const rate = m.successRate !== null ? `${Math.round(m.successRate * 100)}%` : '-';
      console.log(`${m.skillName.padEnd(30)} ${String(m.useCount).padStart(8)} ${String(m.failureCount).padStart(9)} ${rate.padStart(10)} ${last.padStart(8)}`);
    } catch { /* */ }
  }
...
function main(): void {
  ...
  const tokenCount = parseInt(args.includes('--tokens') ? args[args.indexOf('--tokens') + 1] : '0', 10);
  ...
  if (!skillName) { scan(); return; }

  switch (action) {
    case 'increment': increment(skillName, outcome, tokenCount); break;
    case 'fail': recordFailure(skillName, errorType, description); break;
    case 'record': record(skillName, outcome, tokenCount); break;
    default: {
      const metric = readMetric(skillName);
      if (!metric) { saveMetric(skillName, getInitialMetric(skillName)); console.log(`[OK] Created initial metric for ${skillName}`); }
      else console.log(`[OK] ${skillName}: ${metric.useCount} uses, ${metric.failureCount} failures, rate ${metric.successRate}`);
    }
  }
}
