...
if ($Depth -eq "full") {
    ...
    $criticalScripts = @(
        @{ Name = "session-manager.ps1"; SubDir = "SESSION" },
        @{ Name = "detect-tool.ps1"; SubDir = "DETECT" },
        @{ Name = "pre-process-input.ps1"; SubDir = "" },
        @{ Name = "session-autostart.ps1"; SubDir = "SESSION" }
    )
    foreach ($script in $criticalScripts) {
        $scriptPath = if ($script.SubDir) {
            Join-Path $scriptsDir ($script.SubDir + "\" + $script.Name)
        } else {
            Join-Path $scriptsDir $script.Name
        }
        if (-not (Test-Path $scriptPath)) {
            $issues += @{ type = "critical_script_missing"; file = "$($script.SubDir)\$($script.Name)" }
            if (-not $Silent) { Write-Result "ERROR" "Critical script missing: $($script.Name)" }
        }
    }
}
