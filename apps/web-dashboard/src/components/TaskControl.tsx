      ...
      {running.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 border border-blue-200 dark:border-blue-800">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-blue-700 dark:text-blue-300">{running.length} active task(s)</span>
          </div>
          {running.map((t) => (
            <div key={t.id} className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400 ml-6">
      ...
      {recent.length > 0 && (
        <div className="space-y-1">
          {recent.map((t) => {
            ...
            const Icon = cfg.icon;
            return (
              <div key={t.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${cfg.bg}`}>
                <Icon className={`w-4 h-4 ${cfg.color} ${t.status === 'running' ? 'animate-spin' : ''}`} />
                <span className="font-mono text-xs text-gray-500 dark:text-gray-400 w-10">{t.agent}</span>
      ...
      {connected && onEmitEvent && (
        <div className="pt-2 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => onEmitEvent('agent.dispatched', { agent: 'DEV', task: 'quick task', execution_id: `qt-${Date.now()}` })}
            ...
            <button
              onClick={() => onEmitEvent('agent.dispatched', { agent: 'QA', task: 'quick test', execution_id: `qt-${Date.now()}` })}
              className="flex items-center gap-1 px-2 py-1 text-[10px] rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
            >
