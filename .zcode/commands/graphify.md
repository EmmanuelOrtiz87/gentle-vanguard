...
- If the query surfaces a specific node ID, follow up with `npm run graphify -- explain "<node_id>"`.
- Do NOT install the npm package `graphify@1.0.0` (unrelated random graph generator).
- After code modifications in this task, run `npm run graphify -- update .` to validate the snapshot.
