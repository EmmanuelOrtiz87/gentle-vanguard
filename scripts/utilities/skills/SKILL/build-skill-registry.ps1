...
[void]$sb.AppendLine()
[void]$sb.AppendLine("**Auto-generated** | **Skills**: $($skillData.Count)")
[void]$sb.AppendLine()
...
if (Test-Path $registryPath) {
    $existingContent = Get-Content $registryPath -Raw
    # Normalize and trim trailing whitespace for fair comparison
    $existingNorm = ($existingContent -replace "`r`n", "`n" -replace "`r", "`n").TrimEnd()
    $newNorm = ($newContent -replace "`r`n", "`n" -replace "`r", "`n").TrimEnd()
    if ($existingNorm -eq $newNorm) {
        if (-not $Quiet) { Write-Host "[SKILL-REGISTRY] No changes, skipping write" }
        return @{ Status = "SKIPPED"; Path = $registryPath; Count = $skillData.Count }
    }
}
...
Set-Content -Path $registryPath -Value $newContent -Encoding UTF8
