...
export function Tag({ variant = 'neutral', size = 'md', icon, className = '', children, ...rest }) {
  const classes = ['gv-tag', `gv-tag--${variant}`, `gv-tag--${size}`, className]
    .filter(Boolean)
    .join(' ');
  return _jsxs('span', {
    className: classes,
    ...rest,
    children: [
      icon && _jsx('span', { className: 'gv-tag__icon', children: icon }),
      _jsx('span', { className: 'gv-tag__label', children: children }),
    ],
  });
}
