...
export function detectInformalClosure(): GuardianCheck {
  ...
  const stack = new Error().stack || '';
  const hasEngramDirect = stack.includes('mem_session_end') && !stack.includes('session-close-orchestrator');
  const hasManualSave = stack.includes('mem_save') && stack.includes('session') && !stack.includes('session-close-orchestrator');
  
  if (hasEngramDirect || hasManualSave) {
    checks.push({
      passed: false,
      warning: 'Intento de cierre manual detectado: llamada directa a Engram sin orquestador oficial',
      severity: 'HIGH',
      action: 'BLOCK'
    });
  }
  ...
  const hasCloseReport = existsSync(join(sessionDir, 'close-report')) || 
...
export function blockAndRedirect(reason: string): never {
  ...
  writeFileSync(INFORMAL_MARKER, JSON.stringify({
  ...
  const result = runNpxTsxSync('src/session-close-orchestrator.ts', ['--reason', 'redirected-from-informal'], {
    cwd: ROOT,
    stdio: 'inherit',
  });
...
export function markWarning(warning: string, severity: 'LOW' | 'MEDIUM' | 'HIGH'): void {
  ...
  writeFileSync(
    join(ROOT, '.session', 'guardian-warnings.json'),
