...
1. Ejecutar con `-UserInput` (no `-Prompt`):
   ...
   ```
2. El script tiene **response cache** (SHA256, TTL 30min, -33-41% latency).
   Usar `-DisableCache` para bypass cuando sea necesario.
3. Parsear output:
...
- **Autostart pipeline**: 18 steps principales + 6 lazy (self-diagnosis, metrics, dashboard, live-feed, skill-size, adaptive-profile)
...
3. Skills criticos siempre disponibles: `sdd-lifecycle`, `code-review-orchestrator`, `session-workflow`
