...
const taskMultipliers: Record<string, number> = {
  feature: 1.0, bugfix: 0.7, refactor: 0.8, documentation: 0.3,
  test: 0.5, security: 1.2, performance: 0.9, infrastructure: 0.6,
};

const factorMultipliers: Record<string, number> = {
  'external-dependency': 1.3, 'security-impact': 1.5, 'breaking-change': 1.4,
  'cross-platform': 1.2, 'performance-critical': 1.3, 'no-tests': 1.2, 'legacy-code': 1.3,
};
...
function getEstimate(taskType: string, complexity: number, factors: string[], model: string): Estimate {
  ...
  const timeFormatted = estimatedMinutes < 60 ? `${estimatedMinutes} min`
    : estimatedMinutes < 1440 ? `${Math.round(estimatedMinutes / 60 * 10) / 10} hours`
    : `${Math.round(estimatedMinutes / 1440 * 10) / 10} days`;

  const confidence = complexity <= 2 ? 'high' : complexity <= 3 ? 'medium' : 'low';

  return { taskType, complexity, factors: appliedFactors, model, estimatedTime: timeFormatted, estimatedMinutes, estimatedTokens, estimatedCost, confidence };
}
...
function getPRSize(filesChanged: number, linesChanged: number): PRSize {
  ...
  if (linesChanged > 1000 || filesChanged > 30) { size = 'XL'; reviewTime = '2+ hours'; mergeWindow = '3-5 days'; reviewersRequired = 2; }
  else if (linesChanged > 500 || filesChanged > 15) { size = 'L'; reviewTime = '1 hour'; mergeWindow = '2-3 days'; reviewersRequired = 2; }
  else if (linesChanged > 200 || filesChanged > 5) { size = 'M'; reviewTime = '30 min'; mergeWindow = '1-2 days'; reviewersRequired = 1; }
  else if (linesChanged > 50 || filesChanged > 2) { size = 'S'; reviewTime = '15 min'; mergeWindow = 'Same day'; reviewersRequired = 0; }

  return { size, filesChanged, linesChanged, reviewTime, mergeWindow, reviewersRequired };
}

function showReport(taskType: string, complexity: number, factors: string[], model: string, filesChanged: number, linesChanged: number, asJson: boolean, quiet: boolean): Estimate {
  ...
  if (!quiet) {
    ...
    if (prSize.size === 'XL') console.log(`\n[WARN] XL PR detected — MUST split into smaller PRs (< 1000 lines)`);
    if (estimate.confidence === 'low') console.log(`[WARN] Low confidence estimate — consider breaking down into smaller tasks`);
  }
...
function main(): void {
  ...
  const complexity = parseInt(args.includes('--complexity') ? args[args.indexOf('--complexity') + 1] : '3', 10);
  const filesChanged = parseInt(args.includes('--files-changed') ? args[args.indexOf('--files-changed') + 1] : '5', 10);
  const linesChanged = parseInt(args.includes('--lines-changed') ? args[args.indexOf('--lines-changed') + 1] : '200', 10);
  ...
  const factors = factorsStr ? factorsStr.split(',').map(s => s.trim()).filter(Boolean) : [];

  const root = process.env.GENTLE_VANGUARD_BASE_DIR && existsSync(process.env.GENTLE_VANGUARD_BASE_DIR)
    ? process.env.GENTLE_VANGUARD_BASE_DIR : resolve(process.cwd());
  ...
  switch (action) {
    ...
    case 'pr-size': {
      ...
      else console.log(`PR Size: ${prSize.size} | Review: ${prSize.reviewTime} | Merge: ${prSize.mergeWindow} | Reviewers: ${prSize.reviewersRequired}`);
      break;
    }
    case 'velocity': {
