...
function fixProcessExits(filePath: string, dryRun: boolean): FixResult {
  ...
  content = content.replace(
    /catch\s*\([^)]*\)\s*\{[^}]*process\.exit\(1\);/g,
    (match) => match.replace('process.exit(1);', 'throw new Error("Process failed");')
  );
  ...
  content = content.replace(
    /^\s*process\.exit\([01]\);?\s*$/gm,
    ''
  );
  
  // Count changes
  const changes = (originalContent.match(/process\.exit\(/g) || []).length - 
