...
if ($ForceNewMonth -or -not (Test-Path $reportFile)) {
    ...
    } catch { Write-Debug "Exception caught: $_" }
}
...
if (Test-Path $sessionDir) {
    $sessionFiles = Get-ChildItem $sessionDir -Filter "session-*.json" | Where-Objec
        $_.LastWriteTime.Month -eq (Get-Date).Month -and $_.LastWriteTime.Year -eq (Get-Date).Yea
    }
    ...
    foreach ($sessionFile in $sessionFiles) {
        try {
            ...
            if ($engramObservations.Count -gt 0) {
                $sessionObs = $engramObservations | Where-Objec
                ...
                if ($sessionObs -and $sessionObs.Count -gt 0) {
                    ...
                    $skillTitles = $sessionObs | Where-Objec
                    ...
                    $actionTitles = $sessionObs | Where-Objec
                        $_.type -eq 'architecture' -or $_.type -eq 'manual' -o
