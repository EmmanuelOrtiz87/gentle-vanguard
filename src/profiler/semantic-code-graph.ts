...
const SEMANTIC_PATTERNS: Record<string, { type: string; weight: number }[]> = {
  // File patterns
  'session': [{ type: 'IMPLEMENTS', weight: 0.8 }, { type: 'PATTERN:SessionManagement', weight: 0.9 }],
...
function detectType(filePath: string, content: string): 'function' | 'class' | 'interface' | 'module' {
...
function findSemanticEdges(nodes: SemanticNode[]): SemanticEdge[] {
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (target.path.includes('test') && target.path.replace('.test.ts', '.ts').includes(source.path.replace('.ts', ''))) {
        edges.push({
          source: source.id,
          target: target.id,
          type: 'TESTS',
      ...
      try {
        ...
        if (targetContent.includes(`from '${sourceName}'`) || targetContent.includes(`import './${sourceName}'`)) {
          edges.push({
            source: source.id,
            target: target.id,
            type: 'DEPENDS_ON',
...
export async function buildSemanticGraph(): Promise<SemanticGraph> {
  ...
  for (const [moduleName, patterns] of Object.entries(IMPORT_PATTERNS)) {
    const matchingNodes = nodes.filter(n => basename(n.path, '.ts').toLowerCase().includes(moduleName.toLowerCase()));
    ...
    for (const node of matchingNodes) {
      ...
      for (const pattern of patterns) {
        const related = nodes.filter(n => 
          n !== node && 
          n.patterns.some(p => p.toLowerCase().includes(pattern.toLowerCase()))
        );
...
export function querySemanticGraph(query: string): { results: SemanticNode[]; edges: SemanticEdge[] } {
