...
The specific HubSpot objects, properties, and activity types the `crm-maintenance` skill reads from
and writes to. Only the fields listed here are in-scope for this skill — everything else in HubSpot
is untouched.
...
Do not write any other contact properties. Owner, lifecycle stage, and lead source are user-managed
fields — never overwrite.
...
**Never write** `dealstage`, `pipeline`, `hubspot_owner_id`, or any custom property during cleanup.
Those are owner-managed.
...
- If a contact is created on-the-fly during activity logging, associate it to the deal in the same
  operation so the activity shows up on both the contact and deal timelines.
- Do not associate a contact to a deal during an activity-logging flow unless the contact is
  actually a participant in that email thread or meeting.
