...
Describe 'Karpathy Guidelines Tests' {
    BeforeAll {
        ...
        $script:adaptivePath = Join-Path $script:root "scripts/adaptive"
    }

    Context 'Karpathy Enforcer Script' {
        It 'karpathy-enforcer.ps1 exists' {
            $f = Join-Path $script:adaptivePath "karpathy-enforcer.ps1"
        ...
        It 'karpathy-enforcer.ps1 has Simplicity guideline' {
            $f = Join-Path $script:adaptivePath "karpathy-enforcer.ps1"
            $content = Get-Content $f -Raw
            ($content -match 'Simplicity|simple|Simple') | Should -Be $true
        }

        It 'karpathy-enforcer.ps1 has Simplicity keyword' {
            $f = Join-Path $script:adaptivePath "karpathy-enforcer.ps1"
            $content = Get-Content $f -Raw
