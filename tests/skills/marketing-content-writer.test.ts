...
test('marketing-content-writer - SKILL.md exists', () => {
  assert(existsSync(skillPath), 'SKILL.md must exist at .opencode/skills/marketing-content-writer/SKILL.md');
});
...
test('marketing-content-writer - workflow steps are numbered', () => {
  ...
  assert(numberedSteps && numberedSteps.length >= 3, 'Workflow must have at least 3 numbered steps');
});
