...
if ((-not $Force) -and (Test-Path $metaFile)) {
    try { $meta = Get-Content $metaFile -Raw -Encoding UTF8 | ConvertFrom-Json } catch { Write-Debug "Exception caught: #!/usr/bin/env pwsh
param(
    [string]$Project = "",
    [switch]$Force,
    [string]$ExportFile = ""
...
$meta = @{ lastId = 0; indexedAt = ""; count = 0; version = "1.0" }
if ((-not $Force) -and (Test-Path $metaFile)) {
    try { $meta = Get-Content $metaFile -Raw -Encoding UTF8 | ConvertFrom-Json } catch { Write-Output "[ENGRAM-INDEX] Operation failed, continuing" }
}
