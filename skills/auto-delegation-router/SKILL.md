...
description: "Trigger: task routing, subagent delegation, auto-routing, agent selection. Routes tasks to specialized subagents based on keywords, decision trees, and confidence scoring with opt-in control."
license: Apache-2.0
---
...
## Activation Contract
Load when user requests task routing to specialized subagents. Triggers on auto-delegation enable/disable, task-to-agent routing, or when keyword/decision-tree-based agent selection is needed.

## Hard Rules
- NEVER auto-delegate: AGENTS.md/config changes, credential/security ops, session lifecycle management, user-interaction-required tasks
- Auto-delegate appropriate: code (DEV), testing (QA), architecture (SAD), BA, deployment (OPS), script validation (SCRIPT-GOV)
