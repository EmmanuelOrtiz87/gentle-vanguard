  ...
  switch (gate) {
    ...
    case 'pre-commit': {
      ...
      if (candidateSha !== stagedSha) {
        errors.push(`SHA mismatch: receipt=${candidateSha.slice(0, 7)}, current=${stagedSha.slice(0, 7)}`);
    ...
    case 'release': {
      ...
      const criticalCount = (receipt as any).findings?.filter(
    ...
    try {
      switch (action) {
        ...
        case 'status': {
          ...
          const gates: DeliveryGate[] = ['post-apply', 'pre-commit', 'pre-push', 'pre-pr', 'release'];
          
          console.log('RDD Gate Status:\n');
          
          for (const gate of gates) {
            const config = GATE_CONFIGS[gate];
            console.log(`${gate.toUpperCase().padEnd(12)} ${config.required ? '[REQ]' : '[OPT]'} ${config.blockOnFailure ? '[BLOCK]' : '[WARN]'}`);
          }
