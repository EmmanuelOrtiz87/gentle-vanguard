...
function Get-OldWorkflowCheckpoints {
    ...
    foreach ($ref in Get-WorkflowCheckpointRefs) {
        ...
        try { if ([DateTimeOffset]::Parse([string]$dateRaw).LocalDateTime -lt $cutoff) { $oldRefs += $ref } } catch { Write-Debug "Exception caught: function Get-GitInfo {
    $branch = git rev-parse --abbrev-ref HEAD 2>$null
    $status = git status --porcelain 2>$null
    $hasChanges = $status -and $status.Trim() -ne ''
    $ahead = 0; $behind = 0
...
function Ensure-PullRequest {
    ...
    $targetBase = Get-GitFlowBaseForBranch -Branch $Branch
    $existingJson = gh pr view --head $Branch --json number,url,state,baseRefName 2>$null
    if ($LASTEXITCODE -eq 0 -and $existingJson) { try { $pr = $existingJson | ConvertFrom-Json; if ($pr.baseRefName -ne $targetBase) { Write-Error "PR base '$($pr.baseRefName)' != '$targetBase'."; return $null }; return $pr } catch { Write-Debug "Exception caught: function Get-GitInfo {
    $branch = git rev-parse --abbrev-ref HEAD 2>$null
    $status = git status --porcelain 2>$null
    $hasChanges = $status -and $status.Trim() -ne ''
    $ahead = 0; $behind = 0
