...
function Get-ProviderState {
    if (Test-Path $statePath) {
        try { return Get-Content $statePath -Raw | ConvertFrom-Json } catch { Write-Debug "Exception caught: param(
    [ValidateSet('check', 'status', 'route', 'reset')]
    [string]$Action = 'check',
    [string]$AgentType = '',
    [switch]$AsJson,
...
function Get-ProviderState {
    if (Test-Path $statePath) {
        try { return Get-Content $statePath -Raw | ConvertFrom-Json } catch { Write-Output "[FAILOVER] Operation failed, continuing" }
    }
