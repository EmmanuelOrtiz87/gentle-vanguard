...
const AGENT_CONFIG: DomainAgentConfig = {
  ...
  execute: (task, context, prompt) => {
    ...
    const contentType =
      normalized.includes('landing') ? 'LANDING_PAGE' :
      normalized.includes('email') ? 'EMAIL_CAMPAIGN' :
      normalized.includes('blog') || normalized.includes('post') ? 'BLOG_POST' :
      normalized.includes('social') ? 'SOCIAL_MEDIA' :
      normalized.includes('seo') ? 'SEO' :
      normalized.includes('position') || normalized.includes('brand') ? 'POSITIONING' :
      'GENERAL';

    const audience = context?.match(/audience:?\s*([^\n,]+)/i)?.[1]?.trim() || 'Undefined — must be defined before proceeding';
    ...
    if (audience.startsWith('Undefined')) {
      flags.push({ severity: 'critical', message: 'Target audience undefined — refuse to proceed until defined' });
    }
    if (cta === 'Not specified') {
      flags.push({ severity: 'warn', message: 'No CTA detected — every piece needs a clear next step' });
    }

    const artifacts = [
      {
        name: 'content-brief',
        content: `# Content Brief — ${contentType.replace(/_/g, ' ')}\n\n` +
          `**Task:** ${task}\n\n` +
