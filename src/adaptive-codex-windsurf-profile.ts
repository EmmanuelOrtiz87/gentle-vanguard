...
function resolve(dir: string): string { return path.resolve(dir); }
...
function readJson<T>(p: string): T | null {
  try { if (fs.existsSync(p)) return JSON.parse(fs.readFileSync(p, 'utf-8')) as T; } catch { /* ignore */ }
  return null;
}
...
function isPeakHour(timeZone: string, peakStart: number, peakEnd: number): boolean {
  try {
    const formatter = new Intl.DateTimeFormat('en-CA', { timeZone, hour: '2-digit', hour12: false });
    return parseInt(formatter.format(new Date()), 10) >= peakStart && parseInt(formatter.format(new Date()), 10) < peakEnd;
  } catch { return false; }
}

function isTokenPressure(): boolean {
  const budget = readJson<{ used?: number; limit?: number }>(BUDGET_PATH);
  if (typeof budget?.used === 'number' && typeof budget?.limit === 'number') return (budget.used / budget.limit) > 0.8;
  return false;
}
...
function getDefaultState(): AdaptiveState {
  return { optimizationActive: false, normalStreak: 0, lastAction: 'none', lastReason: 'none', lastChangedAt: null };
}
...
function ensureState(state: AdaptiveState | null): AdaptiveState {
  ...
  for (const key of Object.keys(def)) { if (!(key in s)) s[key] = d[key]; }
  return state;
}
