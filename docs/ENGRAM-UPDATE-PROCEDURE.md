...
When updating engram while an MCP client (OpenCode) is running, the binary on disk is updated but
the in-memory MCP subprocess remains the old version. This causes:
...
2. **Engram in PATH** (`$HOME\bin\`) vs **Engram in scripts/utilities/**
   (`workspace-foundation/scripts/utilities/`)
...
> `engram setup opencode`
>
> Then restart Claude Code so it reloads the Engram MCP subprocess and refreshed hook/config files.
