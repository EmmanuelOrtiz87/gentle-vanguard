...
function findMarkdownFiles(dir: string, files: string[] = []): string[] {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && (extname(entry.name) === '.md' || extname(entry.name) === '.yml' || extname(entry.name) === '.json')) {
      files.push(fullPath);
    }
  }
...
function main(): void {
  ...
  console.log(`║  Mode: ${dryRun ? 'DRY RUN (no changes)' : 'LIVE (will write files)'}                            ║`);
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log();
