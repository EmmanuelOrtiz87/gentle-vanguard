...
Score each customer using QuickBooks payment history for the last 12 months. Require a minimum of 3
invoices to score; fewer than 3 defaults to `occasionally-late`.
...
Every reminder includes: invoice number(s), total amount due, original due date, days overdue, and
...
If a customer has multiple overdue invoices, combine into one email. List each invoice (number,
amount, due date), then state the combined total. Use the customer's score, not the most overdue
invoice's score.
