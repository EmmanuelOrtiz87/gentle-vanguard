...
const AGENT_CONFIG: DomainAgentConfig = {
  ...
  execute: (task, context, prompt) => {
    ...
    const intent =
      normalized.includes('qualif') ? 'QUALIFY' :
      normalized.includes('pipeline') || normalized.includes('forecast') ? 'PIPELINE' :
      normalized.includes('outreach') || normalized.includes('email') ? 'OUTREACH' :
      normalized.includes('account') || normalized.includes('plan') ? 'ACCOUNT_PLAN' :
      normalized.includes('deal') || normalized.includes('review') ? 'DEAL_REVIEW' :
      'GENERAL';

    // Extract optional MEDDIC context hints: metric, buyer, champion, pain
    const extract = (label: string): string =>
      context?.match(new RegExp(`${label}:?\\s*([^\\n,]+)`, 'i'))?.[1]?.trim() || 'Not captured — required';
    ...
    const artifacts = [
      {
        ...
        content: `# Deal Qualification — ${intent}\n\n` +
          `**Task:** ${task}\n\n` +
          `## MEDDIC\n\n` +
          Object.entries(meddic).map(([k, v]) => `- **${k}:** ${v}`).join('\n') + '\n\n' +
          `## Required Evidence\n\n` +
          `- MEDDIC qualification documented\n- Champion identified and engaged\n- Economic buyer contacted\n- Decision criteria documented\n- Next action defined with date\n`
      }
    ];
