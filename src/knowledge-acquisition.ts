...
async function fetchUrl(url: string, timeout = 30000): Promise<{ success: boolean; content?: string; contentType?: string; error?: string }> {
  try {
...
function fetchUrlSync(url: string, timeout = 30000): { success: boolean; content?: string; contentType?: string; error?: string } {
  // For CLI usage, use curl as fallback
  try {
    const isWindows = process.platform === 'win32';
    const cmd = isWindows ? 'powershell.exe' : 'curl';
    const args = isWindows 
      ? ['-Command', `Invoke-WebRequest -Uri "${url}" -TimeoutSec ${Math.floor(timeout / 1000)} -UseBasicParsing | Select-Object -ExpandProperty Content`]
      : ['-s', '-L', '--max-time', String(Math.floor(timeout / 1000)), '-H', 'User-Agent: Gentle-Vanguard/1.0', url];
...
function integrateIntoLearningEngine(entry: any): void {
  ...
  try {
    runNpxTsxSync('src/learning-engine.ts', ['--integrate', JSON.stringify({
      source: entry.source,
      title: entry.title,
...
function showHelp(): void {
  ...
  console.log('  npx tsx src/knowledge-acquisition.ts --fetch https://example.com/docs --source docs');
